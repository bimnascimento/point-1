...
url:"<?php echo $arfajaxurl ?>",params:"action=arfupdatefieldname", value_required:"true", desc:def_title, success:function(){ 
	...
	change_password_field_dropdown(); 
...
jQuery(document).ready(function(){
    ...
    jQuery('#arfmainfieldid_<?php echo $field['id']; ?> .arfhelptip').tipso({
        position:'top',
        maxWidth:'400',
        useTitle:true,
        background:'#444444',
...
<?php if( $field['type'] == 'file' || $field['type'] == 'divider' || $field['type'] == 'arfslider' || $field['type'] == 'like' || $field['type'] == 'colorpicker' ) { ?>
jQuery('#arfmainfieldid_<?php echo $field['id']; ?> .arf_coloroption_sub:not(.arf_clr_disable)').colpick({
    ...
    onSubmit: function () {
        jQuery('#arfmainfieldid_<?php echo $field['id']; ?> .arf_coloroption_sub:not(.arf_clr_disable)').colpickHide();
    }
});
<?php } ?>
</script>
