    ...
    if (!is_admin() && apply_filters('arf_check_for_draw_outside', false, $field)) {
        ...
        if (is_array($field['value'])) {
            foreach ($field['value'] as $checked) {
                ...
                ?><input type="hidden" name="<?php echo $field_name ?>[]" value="<?php echo esc_attr($checked) ?>"/><?php
            }
        } else {
            $hidden_field_value =  $field['value'];
            ...
            $hiddenvalue = '<input type="hidden" id="field_' . $field['field_key'] . '" name="' . $field_name . '" value="' . esc_attr($hidden_field_value) . '"/>';
        ...
        foreach ($previous_fields as $prev){ 
            ...
            if (isset($_POST['item_meta'][$prev->id])){ 
                ...
                if (is_array($_POST['item_meta'][$prev->id])){
                    ...
                    ?><input type="<?php echo apply_filters('arfpagedfieldtype', 'hidden', array('field' => $prev)) ?>" id="field_<?php echo $prev->field_key ?>" name="item_meta[<?php echo $prev->id ?>]" value="<?php echo stripslashes(esc_html($_POST['item_meta'][$prev->id])) ?>" /><?php
                }
            }
        }
    } else {
