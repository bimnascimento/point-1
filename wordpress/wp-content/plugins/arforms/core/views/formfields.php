...
global $wpdb, $armainhelper, $arfieldhelper, $arformcontroller, $arrecordcontroller, $arf_font_awesome_loaded;
...
?><div class="controls arf_date_main_controls arf_cal_theme_<?php echo $newarr['arfcalthemecss']; ?>" <?php if( isset($field['field_width']) and $field['field_width']!='') { echo 'style="width:'.$field['field_width'].'px;"' ; } ?>><?php
...
$arfdatepickerloaded['field_'. $field['field_key']] = array(
    ...
    'locale' => $field['locale'],  'entry_id' => $entry_id,

    'field_id' => $field['id'], 'date_format' => $newarr['date_format'],
	
    'show_year_month_calendar' => $field['show_year_month_calendar'],
    
    'start_date' => $field['start_date'], 'end_date' => $field['end_date'],
    
    'off_days' => $field['off_days'], 'show_time_calendar' => $field['show_time_calendar'],
    
    'arf_show_min_current_date' => $field['arf_show_min_current_date'], 'arf_show_max_current_date' => $field['arf_show_max_current_date'],
...
?>
<div class="sltstandard_time controls arf_time_main_controls arf_cal_theme_<?php echo $newarr['arfcalthemecss']; ?> " <?php if( isset($field['field_width']) and $field['field_width']!='') { echo 'style="width:'.$field['field_width'].'px;"' ; } ?>>
<?php
if(apply_filters('arf_check_for_draw_outside',false,$field))
{
	do_action('arf_drawthisfieldfromoutside',$field);
...
$arftimepickerloaded['field_'. $field['field_key']] = array(
    ...
    'step' => $field['step'], 'start_time' => $field['start_time'], 'end_time' => $field['end_time'], 'default_hour' => $field['default_hour'], 'default_minutes' => $field['default_minutes'],'entry_id' => $entry_id);
...
if( $field['type'] == 'confirm_password' ){ ?>data-validation-match-match="item_meta[<?php echo $field['confirm_password_field']; ?>]" data-cpass="1" data-validation-match-message="<?php echo esc_attr($field['invalid']); ?>" class="arf_password_field" <?php } ?> 

<?php
echo $arfieldhelper->get_onchage_func($field); 
if( $field['type'] == 'confirm_email' ){ ?>data-validation-match-match="item_meta[<?php echo $field['confirm_email_field']; ?>]" data-cpass="1" data-validation-match-message="<?php echo esc_attr($field['invalid']); ?>"  <?php } if(wp_is_mobile()){ ?> data-mask-input="<?php echo $inputmask;?>" data-ismask="true" <?php } ?>
...
?><div class="file_main_control" style="display:inline-block; <?php if( isset($field['field_width']) and $field['field_width']!='' ){ echo "width:".$field['field_width'].'px'; } ?>">
        <div class="arf_file_field"><div class="<?php if( $browser_info['name'] == 'Internet Explorer' and $browser_info['version'] <= '9' ){ echo 'original_btn'; }?> arfajax-file-upload" id="div_<?php echo $field['field_key'] ?>" data-id="<?php echo $field['id'] ?>" data-form-id="<?php echo $field['form_id'];?>" style="position: relative; overflow: hidden; float:left; cursor:pointer;"><?php echo '<div class="arfajax-file-upload-img" style="float:left;">&#32;</div>&#32;'.$field['file_upload_text']; 
                if( $browser_info['name'] == 'Internet Explorer' and $browser_info['version'] <= '9' ){
                    ...
                    ?><input type="text" <?php if(isset($field['required']) and $field['required']){ echo 'data-validation-required-message="'.esc_attr($field['blank']).'"'; }?> class="original" name="file<?php echo $field['id'] ?>" id="field_<?php echo $field['field_key'] ?>" <?php do_action('arffieldinputhtml', $field) ?> data-form-id="<?php echo $field['form_id'];?>" data-file-valid="true" data-invalid-message="<?php echo esc_attr($field['invalid']);?>" style="position: absolute; cursor: pointer; top: 0px; width: 100%; height:100%; left: -999px; z-index: 100; opacity: 0; filter:alpha(opacity=0);" /><?php
                ...
                ?><input type="file" <?php if(isset($field['required']) and $field['required']){ echo 'data-validation-required-message="'.esc_attr($field['blank']).'"'; }?> class="file original" name="file<?php echo $field['id'] ?>" id="field_<?php echo $field['field_key'] ?>" data-invalid-message="<?php echo esc_attr($field['invalid']);?>" data-form-id="<?php echo $field['form_id'];?>" data-file-valid="true" style="position: absolute; cursor: pointer; top: 0px; padding:0; margin:0; height:100%; outline:none; right:0; z-index: 100; opacity: 0; width:100%" />
...
?></div>
            <div id="remove_<?php echo $field['field_key'] ?>" class="ajax-file-remove" style="display:none; position: relative; overflow: hidden; float:left;" data-form-id="<?php echo $field['form_id']; ?>" data-id="<?php echo $field['id']; ?>" ><img src="<?php echo ARFIMAGESURL.'/remove-icon.png';?>" alt="<?php _e('Remove', 'ARForms'); ?>"/> &#32;<?php echo $field['file_remove_text'];?></div>
