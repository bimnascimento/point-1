...
global $style_settings, $armainhelper, $arfieldhelper, $arformcontroller, $arf_font_awesome_loaded;
...
<li style=" <?php echo $myliclass . $multicolborder; ?> margin:0px;" id="arfmainfieldid_<?php echo $field['id']; ?>" class="arfmainformfield edit_form_item arffieldbox ui-state-default <?php echo $display['options'] ?> edit_field_type_<?php echo $display['type'] ?> top_container <?php echo $multicolclass; ?>" onmouseover="arffieldhover(1,<?php echo $field['id']; ?>)" onmouseout="arffieldhover(0,<?php echo $field['id']; ?>)">
    ...
    <div class="arf_fieldiconbox">
        <div class="arf_field-icon arfshowiconsonhover<?php echo $field['id']; ?>" style="visibility:hidden;">
            <?php if ($field['type'] != 'hidden') { ?>
                <div class="arffield-iconbox">
                    <a href="javascript:void(0);" onclick="arfshowfieldoptions('<?php echo $field['id']; ?>')" id="field-setting-button-<?php echo $field['id']; ?>" class="frm-setting arfhelptip" title="<?php _e('Field Options', 'ARForms'); ?>" data-lower="false"><img id="field-setting-icon<?php echo $field['id']; ?>" src="<?php echo ARFIMAGESURL ?>/editor_icons/setting-icon_hover.png" alt="Duplicate" /></a> 
                </div>
            <?php } ?>
    ...
    <div class="allfields">    
        ...
        if ($display['type'] == 'text') {
            ...
            if ($display['field_data']['enable_arf_prefix'] == 1 or $display['field_data']['enable_arf_suffix'] == 1) {
                ...
                if ($display['field_data']['enable_arf_prefix'] == 1) {
                    echo "<span class='arf_editor_prefix' id='arf_editor_prefix_{$field['id']}'><i class='arfa " . $display['field_data']['arf_prefix_icon'] . "'></i></span>";
                }
                $input_cls = 'arf_prefix_suffix';
            }
            ?>
            <input type="text" class="textbox <?php echo $input_cls . ' ' . $inp_cls; ?>" name="<?php echo $field_name ?>" id="itemmeta_<?php echo $field['id']; ?>" onkeyup="arfchangeitemmeta('<?php echo $field['id']; ?>');" value="<?php echo esc_attr($field['default_value']); ?>"  /> 

            <?php
            if ($display['field_data']['enable_arf_prefix'] == 1 or $display['field_data']['enable_arf_suffix'] == 1) {
                $arf_font_awesome_loaded = 1;
                if ($display['field_data']['enable_arf_suffix'] == 1) {
                    echo "<span class='arf_editor_suffix' id='arf_editor_suffix_{$field['id']}'><i class='arfa " . $display['field_data']['arf_suffix_icon'] . "'></i></span>";
                }
                echo "</div>";
            }
            ?>
        ...
        <?php } else if ($display['type'] == 'colorpicker') { ?>
            <div class="arf_editor_prefix_suffix_wrapper arf_editor_colorpicker">
                <span class="arf_editor_prefix"  id='arf_editor_prefix_<?php echo $field['id']; ?>'><i class="arfa arfa-paint-brush"></i></span>
        ...
        <?php } else if ($field['type'] == 'textarea') { ?>
            ...
            <textarea name="<?php echo $field_name ?>" id="itemmeta_<?php echo $field['id']; ?>" onkeyup="arfchangeitemmeta('<?php echo $field['id']; ?>');" rows="<?php echo $field['max_rows']; ?>"><?php echo $armainhelper->esc_textarea($field['default_value']); ?></textarea> 
            ...
            <div class="arf_checkbox_div" id="arf_checkbox_div_<?php echo $field['id']; ?>">
                ...
                ?>">
                             ...
                             foreach ($field['options'] as $opt_key => $opt) {
                                ...
                                if($field['type'] == 'checkbox' ){
                                    if(isset($field['value'])&&is_array($field['value'])){
                                        ...
                                        $checked = (isset($field['value']) and ( (!is_array($field['value']) && $field['value'] == $field_val ) || (isset($field['value']) && is_array($field['value']) && in_array($field_val, $field['value'])))) ? ' checked="true"' : '';
                                    }
                                }else{
                    ...
                    <div style="display: none; visibility: hidden;" id="arf_op_hidden_fieldcheck_sub_<?php echo $field['id'] ?>">
                            <?php
                            $total_options = count($field['options']);
                            foreach ($field['options'] as $opt_key => $opt) {
                                $field_val = apply_filters('arfdisplaysavedfieldvalue', $opt, $opt_key, $field);
                            ...
                            if($field['type'] == 'checkbox' ){
                                ...
                                ?>
                                <input type="<?php echo $field['type'] ?>" id="arf_op_fieldcheck_sub_<?php echo $field['id'] ?>-<?php echo $opt_key ?>" name="<?php echo $field_name ?><?php echo ($field['type'] == 'checkbox') ? '[]' : ''; ?>"  data-label="<?php echo esc_attr($opt); ?>"  value="<?php echo esc_attr($field_val) ?>"<?php echo isset($checked) ? $checked : ''; ?>/>
                                <?php
                                unset($checked);
                            }
                            ?>
            ...
            <div style="right:0; margin-top:-100px; position:absolute;" id="main_fieldoptions_modal_opt_<?php echo $field['id']; ?>" class="main_fieldoptions_modal">
                <div id="arffieldoptions_<?php echo $field['id'] ?>" class="show-field-options arfmodal" style="display:none;margin-top:0px;top:0px;position:absolute;width:auto;min-width: 200px;">
                    ...
                    <div class="arfmodal-header"><div><?php _e('Edit Options', 'ARForms'); ?>
                    ...
                    <div class="arfmodal-body_fieldoptions">
                        ...
                        <form id="arf_op_temp_form_<?php echo $field['id'] ?>" action="#">&nbsp;</form>
                        <div style="width:auto;">
                            
                            
                            <div id="arf_field_<?php echo $field['id'] ?>_opts" class="clear <?php echo (count($field['options']) > 5) ? ' arffieldoptionslist' : ' arffieldoptionslist'; ?>"  style="width:<?php echo ($field['separate_value'] == 1) ? '96%' : '410px';   ?>;">
                                <?php do_action('arfaddsepratevalues', $field); ?>
                                <div id="arf_field_op_html_<?php echo $field['id'] ?>">
                                    <form id="arf_op_form_<?php echo $field['id'] ?>" action="#">
                                        <span style="float: left; display: none; margin: 20px 0 0 0; text-align:center; width:99% " id="arf_field_op_html_loder_<?php echo $field['id']; ?>"><img src="<?php echo ARFURL; ?>/images/ajax_loader_gray_32.gif" /></span>
                                    </form>
        ...
        <?php foreach ($prepop as $label => $pop) { ?>
                                                ...
                                                if(array_keys($pop) !== range(0, count($pop) - 1))
                                                {
                                                   
                                                    $final_pop_data = array();
                                                    foreach($pop as $new_key=>$new_data)
                                                    {
                                                        ...
                                                        $final_pop_data[] = htmlspecialchars($new_key_data, ENT_QUOTES, 'UTF-8').'|'.htmlspecialchars($new_data, ENT_QUOTES, 'UTF-8');
                                                    }
                                                    ...
                                                    <li class='arf_selectbox_option' data-value='<?php echo json_encode(array_values($final_pop_data)) ?>' data-label='<?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?>'><?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8'); ?></li>
        <?php } ?>                            
                                            </ul>
                                        </dd>
                                    </dl>
                                ...
                                <div class="arf_preset_field_content_wrapper arf_preset_field_content_wrapper_<?php echo $field['id'] ?>">
                                    <div class="arf_preset_data_inner">
                                        <input type="file" class="arf_preset_data arf_preset_data_<?php echo $field['id'] ?>" id="arf_preset_data_<?php echo $field['id'] ?>" name="arf_preset_data" data-val="<?php echo $field['id'] ?>"/>
                                        <div class="arf_preset_data_text"><?php _e('Upload only CSV file.', 'ARForms') ?></div>
                                    </div>
                                    <input type="checkbox" value="1" class="arf_preset_future_use" name="arf_preset_future_use" id="arf_preset_future_use_<?php echo $field['id'] ?>"/>
                                    <label for="arf_preset_future_use_<?php echo $field['id'] ?>">Save for future use</label>
                                    <input type="text" name="arf_preset_title" class="arf_preset_title arf_preset_title_<?php echo $field['id'] ?>" placeholder="Preset title" >
                                    <a href="javascript:void(0)" onclick="arf_update_preset_data('<?php echo $field['id'] ?>')"  style="color: #21759B;" class="arlinks">
                                        <button class="btn_2" type="button" style="height: 30px; font-size: 15px; line-height: 23px; margin-left: 10px;"><?php _e('Save', 'ARForms') ?></button>
                                    </a>
                                </div>
                        ...
                        <div style="clear:both;"></div>
                        <div align="right" class="arfpopupclosediv"> 
                            <span style="margin-top:6px; float:left; display: none;" id="arf_op_save_option_loder_<?php echo $field['id']; ?>"><img src="<?php echo ARFURL; ?>/images/ajax-loader1.gif" /></span>
                            <button type="button" class="arfpopupsave" onclick="arf_op_save_option('<?php echo $field['id']; ?>','<?php echo $field['ref_field_id']; ?>');" ><?php _e('Save', 'ARForms'); ?></button> 
                            <button type="button" class="arfpopupclose" data-dismiss="arfmodal" onclick="close_add_field_subject('arffieldoptions_<?php echo $field['id']; ?>')" ><?php _e('Cancel', 'ARForms'); ?></button> 
                        </div>
        ...
        <?php } else if ($field['type'] == 'select') { ?>
                ...
                foreach ($field['options'] as $opt_key => $opt) {
                    ...
                    ?>
                        ...
                        <option value="<?php echo htmlspecialchars($field_val) ?>"<?php echo $selected ?>><?php echo htmlspecialchars($opt); ?></option>
                ...
                <div id="arffieldoptions_<?php echo $field['id'] ?>" class="show-field-options arfmodal" style="display:none;margin-top:0px;top:0px;position:absolute;width:auto;min-width: 200px;">
                    ...
                    <div class="arfmodal-body_fieldoptions">
                        ...
                        <div class="arfshowfieldclick">
                            <form id="arf_op_temp_form_<?php echo $field['id'] ?>" action="#">&nbsp;</form>
                            <div id="arf_field_<?php echo $field['id'] ?>_opts"<?php echo (count($field['options']) > 5) ? ' class="arffieldoptionslist"' : ' class="arffieldoptionslist"'; ?> style="width:<?php echo ($field['separate_value'] == 1) ? '100%' : '410px';   ?>;">
                                <?php do_action('arfaddsepratevalues', $field); ?>
                                <div id="arf_field_op_html_<?php echo $field['id'] ?>">
                                    <form id="arf_op_form_<?php echo $field['id'] ?>" action="#">
                                        <span style="float: left; display: none; margin: 20px 0 0 0; text-align:center; width:99% " id="arf_field_op_html_loder_<?php echo $field['id']; ?>"><img src="<?php echo ARFURL; ?>/images/ajax_loader_gray_32.gif" /></span>
                            ...
                            <div id="frm_add_field_<?php echo $field['id']; ?>"> <br  />
                                ...
                                <div style="float:left; padding-right:10px; padding-top:3px;">
                                    ...
                                    <button onclick="javascript:arfaddnewfieldoption(<?php echo $field['id']; ?>)" id="arfaddanoption_<?php echo $field['id']; ?>" class="btn_2" type="button" style="height:26px;font-size:12px;margin:0px 5px 5px 0px;line-height:23px;padding:0 10px;<?php                                    if (!is_array($field['options']) || !($field['options'])) {
                                        echo 'display:inline;';
                                    } else {
                                        echo 'display:none;';
                                    }
        ...
        <?php foreach ($prepop as $label => $pop) { ?>
                                                ...
                                                if(array_keys($pop) !== range(0, count($pop) - 1))
                                                {
                                                    ...
                                                    foreach($pop as $new_key=>$new_data)
                                                    {
                                                        ...
                                                        ?>
                                                        <li class='arf_selectbox_option' data-value='<?php echo json_encode($final_pop_data) ?>' data-label='<?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?>'><?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8') ?></li>
                                    <?php } ?>                            
                                                </ul>
                                            </dd>
                                        </dl>
                                ...
                                <div class="arf_preset_field_content_wrapper arf_preset_field_content_wrapper_<?php echo $field['id'] ?>">
                                    ...
                                    <input type="checkbox" class="arf_preset_future_use" value="1" name="arf_preset_future_use" id="arf_preset_future_use_<?php echo $field['id'] ?>"/>
                                    <label for="arf_preset_future_use_<?php echo $field['id'] ?>">Save for future use</label>
                                    <input type="text" name="arf_preset_title" class="arf_preset_title arf_preset_title_<?php echo $field['id'] ?>" placeholder="Preset title">
                                    <a href="javascript:void(0)" onclick="arf_update_preset_data('<?php echo $field['id'] ?>')"  style="color: #21759B;" class="arlinks">
                                        <button class="btn_2" type="button" style="height: 30px; font-size: 15px; line-height: 23px; margin-left: 10px;"><?php _e('Save', 'ARForms') ?></button>
            ...
            <?php if (empty($arfsettings->pubkey) && $field['is_recaptcha'] != 'custom-captcha') { ?>
                ...
                <div class="howto" id="setup_captcha_message" style="font-weight:bold;color:red;line-height:1;font-size:11px;"><?php echo __('Please setup site key and private key in Global Settings otherwise recaptcha will not appear', 'ARForms') ?></div>
                    ...
                    <?php if ($field['type'] != 'hidden') {
                        ...
                        <div id="main_fieldoption_table" class="fields_table">  
                            ...
                            <div class="fieldoptions_tab_main">
                                ...
                                <div style=" <?php
                                if ($field['type'] == 'imagecontrol' || $field['type'] == 'captcha' ) {
                                    echo 'display:none;';
                                }
                                            ...
                                            if ($totaloptions > 0) {
                                                ...
                                                foreach ($options as $k => $v) {
                                                    ...
                                                    if ($k == "multicolsetting")
                                                        ...
                                                        $mainwidth = "29%";
                                                    else if ($field['type'] == 'date' && $k == "daterange")
                                                        $mainwidth = "64%";
                                    ...
                                    <div style="margin-top:20px;">
                                        ...
                                        <div style="margin-left:100px;">
                                            <div id="custom_css_btns_<?php echo $field['id'] ?>" style="margin-bottom:20px;">
                                                ...
                                                if ($field['type'] == 'like' || $field['type'] == 'arfslider' || $field['type'] == 'divider')
