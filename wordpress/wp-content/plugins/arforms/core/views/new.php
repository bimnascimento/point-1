<?php
global $arformcontroller, $armainhelper, $arfieldhelper, $arrecordcontroller,$wpdb,$arfversion;
...
<form enctype="<?php echo apply_filters('arfformenctype', 'multipart/form-data', $form) ?>" method="post" class="arfshowmainform arfpreivewform <?php echo $temp_calss;?> <?php do_action('arfformclasses', $form) ?>" id="form_<?php echo $form->form_key ?>" novalidate="" data-id='<?php echo $arf_data_uniq_id; ?>' data-popup-id='<?php echo isset($arf_popup_data_uniq_id) ? $arf_popup_data_uniq_id : ''; ?>'>
<?php } else {  ?>
<form enctype="<?php echo apply_filters('arfformenctype', 'multipart/form-data', $form) ?>" method="post" class="arfshowmainform <?php echo $temp_calss;?> <?php do_action('arfformclasses', $form) ?>" id="form_<?php echo $form->form_key ?>" <?php echo ($arfsettings->use_html) ? '' : 'action=""'; ?> novalidate="" data-id='<?php echo $arf_data_uniq_id; ?>' data-popup-id='<?php echo isset($arf_popup_data_uniq_id) ? $arf_popup_data_uniq_id : ''; ?>'>
    <?php }  ?>
...
?></form><?php
	...
	if( ( isset($preview) and $preview ) or (isset($_POST['is_preview']) and $_POST['is_preview']=='yes') ) { 
		...
		if($arfsettings->form_submit_type == 1)  
			 wp_enqueue_script('filedrag', ARFURL.'/js/filedrag/filedrag_front.js',array(),$arfversion);						
