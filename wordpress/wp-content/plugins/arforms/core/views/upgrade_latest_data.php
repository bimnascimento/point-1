...
if(version_compare($newdbversion, '1.0', '>') || version_compare($newdbversion, '1', '='))
{
        ...
        if(version_compare($newdbversion, '2.7.4', '<'))
	{
		...
		foreach($res as $key => $val) 
		{
			...
			foreach($form_fields as $key => $val) 
			{
				...
				$res = $wpdb->update( $wpdb->prefix."arf_fields", array('type' => $fieldtype, 'field_options' => $optionsnewval ), array( 'id' => $val->id ) );
			}
        ...
        if(version_compare($newdbversion, '2.7.7', '<')){
            global $wpdb, $MdlDb;
	...
	update_option('arf_db_version','2.7.8');
