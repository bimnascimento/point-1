...
global $arforms_loaded,$arfversion;
$arforms_loaded[$form->id] = isset($arforms_loaded[$form->id])?$arforms_loaded[$form->id]:'';
	...
	foreach($values['fields'] as $field)
	{
		if( $field['type'] == 'imagecontrol' )
		{
			...
			if( $field['image_url'] != '' )
			{
				...
				echo '<div id="arf_imagefield_'.$field['id'].'" class="arf_image_field '.$arfimagealignclass.'" '.$arfimagealign.' style="'.$arfimageleft.$arfimagetop.'"><img src="'.$field['image_url'].'" style="'.$arfheightwidth.'" alt="" /></div>';
...
if($values['fields'] and $page_num > 0 ){
	...
	foreach($values['fields'] as $field){
		...
		$field['id'] = $arfieldhelper->get_actual_id($field['id']);
		if($field_type=="break") {
			$first_page_break_field_val = $field;
...
foreach($values['fields'] as $field){
	...
	{
        do_action('arfdisplayfieldtype1', $field, $form, $page_num, $arf_data_uniq_id);
	}
...
if($arf_page_number > 0 and $page_num > 0 ) {
	echo '<input type="button" value="'.$field_pre_page_title.'" '.$display_previous.' name="previous" id="previous_last" class="previous_btn" onclick="go_previous(\''.($arf_page_number-1).'\', \''.$form->id.'\', \'no\', \''.$form->form_key.'\', \''.$arf_data_uniq_id.'\');"  />';
...
if($arf_page_number > 0 and $page_num > 0 ) {
	...
	echo '<input type="hidden" value="'.$is_submit_form.'" data-val="1" data-hide="'.$page_break_hidden_array[$form->id]['data-hide'].'" data-max="'.$arf_page_number.'" name="submit_form_'.$form->id.'" id="submit_form_'.$form->id.'" />';
