...
class arrecordhelper{
	...
	function display_value($value, $field, $atts=array()){
        ...
        if ($atts['type'] == 'image'){
            ...
            $value = $arfieldhelper->get_date($value, $date_format= false, $field->id);
			...
			if( $value != '' ) {
				$class = ($value == '1') ? 'arf_like_btn' : 'arf_dislike_btn';	
                                $like_bgcolor = ($value == '1') ? $field->field_options['like_bg_color'] : $field->field_options['dislike_bg_color'];
				
				if($value == '1')
					$value = '<label style="margin-left:0;background:'.$like_bgcolor.';" class="'.$class.' active"><img src="'.ARFURL.'/images/like-icon.png" alt="'.__('Like', 'ARForms').'" /></label>';
				else
					$value = '<label style="margin-left:0;background:'.$like_bgcolor.';" class="'.$class.' active"><img src="'.ARFURL.'/images/dislike-icon.png" alt="'.__('Dislike', 'ARForms').'" /></label>';					
			}
		}
