...
class arfieldcontroller {
    ...
    function __construct() {
        ...
        add_action('arfdisplayfieldtype1', array(&$this, 'show_other'), 10, 4);
        ...
        add_action('wp_ajax_arf_op_updateoptionorder', array(&$this, 'arf_op_updateoptionorder'));
    }
    ...
    function show_other($field, $form, $total_page = 0, $current_form_data_id = 0) {
    ...
    function add_separate_value_opt_label($field) {
        ...
        $style_label = $field['separate_value'] ? "" : "style='width:350px;'";

        echo '<div class="arfshowfieldclick">';


        echo '<div class="field_' . $field['id'] . '_option_key frm_option_val_label" ' . $style_label . '>' . __('Option Label', 'ARForms') . '</div>';
    ...
    function input_fieldhtml($field, $echo = true) {
        ...
        if (isset($field['max']) and ! empty($field['max']))
    ...
    function arf_save_field_option() {
        ...
        $ref_field_id = (isset($_POST['ref_field_id']) && !empty($_POST['ref_field_id'])) ? $_POST['ref_field_id'] : $field_id;
        ...
        $arf_op_label = $arf_op_value = $options = array();
        $default_val = '';
        if (!empty($field_id)) {
            ...
            if (isset($separate_value) && $separate_value == 'true') {
                foreach ($arf_op_label[$field_id] as $k => $label) {
                    $options[] = array('value' => $arf_op_value[$field_id][$k], 'label' => $label);
                }
            }else{
            ...
            $update = $wpdb->update($wpdb->prefix . 'arf_fields', array('options' => maybe_serialize($options),'default_value' => $default_val), array('ref_field_id' => $ref_field_id), array('%s'), array('%d'));
            echo $update;
        }
        die();
    ...
    function arf_field_op_lord_html() {
     ...
     function arf_op_updateoptionorder() {
        global $wpdb;
        ...
        $wpdb->update($wpdb->prefix . 'arf_fields', array('option_order' => $order), array('ref_field_id' => $ref_field_id), array('%s'), array('%d'));
