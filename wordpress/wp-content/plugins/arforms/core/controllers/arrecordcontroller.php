...
class arrecordcontroller {
    ...
    function register_scripts() {
        ...
        wp_register_script('jquery-frm-rating', ARFURL . '/js/jquery.rating.min.js', array('jquery'), $arfversion, true);


        wp_register_script('jquery-star-metadata', ARFURL . '/js/jquery.MetaData.js', array('jquery'), $arfversion, true);
    ...
    function show_form_popup($id = '', $key = '', $title = false, $description = false, $desc = '', $type = 'link', $modal_height = '540', $modal_width = '800', $position = 'left', $btn_angle = '0', $bgcolor = '', $txtcolor = '', $open_inactivity = '1', $open_scroll = '10', $open_delay = '0', $overlay = '0.6', $is_close_link = 'yes',$modal_bgcolor = '#000000') {
        ...
        if ($form->is_loggedin and $user_ID and isset($form->options['logged_in_role']) and $form->options['logged_in_role'] != '') {
            ...
            if ($armainhelper->user_has_permission($form->options['logged_in_role'])) {
                ...
                return $arrecordcontroller->get_form_popup(VIEWS_PATH . '/view-modal.php', $form, $title, $description, $desc, $type, $modal_height, $modal_width, $position, $btn_angle, $open_inactivity, $open_scroll, $open_delay, $overlay, $is_close_link,$modal_bgcolor);
            } else {
            ...
            return $arrecordcontroller->get_form_popup(VIEWS_PATH . '/view-modal.php', $form, $title, $description, $desc, $type, $modal_height, $modal_width, $position, $btn_angle, $bgcolor, $txtcolor, $open_inactivity, $open_scroll, $open_delay, $overlay, $is_close_link,$modal_bgcolor);
    }

    function get_form_popup($filename, $form, $title, $description, $desc, $type, $modal_height, $modal_width, $position, $btn_angle, $bgcolor, $txtcolor, $open_inactivity, $open_scroll, $open_delay, $overlay, $is_close_link,$modal_bgcolor) {
                ...
                if ($bulkaction == 'delete') {
                    ...
                    if (!current_user_can('arfdeleteentries')) {
                        ...
                        if (is_array($items)) {
                            ...
                            if ($del_res)
                                $message = __('Entries deleted successfully', 'ARForms');
                        }
                    }
                }else if ($bulkaction == 'csv') {
    ...
    function frm_change_entries($new_form_id = '', $new_start_date = '', $new_end_date = '', $bulk = '', $message = '', $errors = '') {
        ...
        <form method="get" id="list_entry_form" class="arf_list_entries_form" onsubmit="return apply_bulk_action();" style="float:left;width:100%;">
            ...
            <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
                ...
                <th class="arf_col_action"><?php _e('Action', 'ARForms'); ?></th>          
                </tr>
                </thead>
                <tbody>
            ...
            foreach ($items as $key => $item) {
                ?>    
                            <tr>
                                ...
                                <td><?php $browser_info = $this->getBrowser($item->browser_info);
    ...
    function arfentryactionfunc() {
        ...
        if ($_REQUEST['act'] == 'delete' and $_REQUEST['id'] != '') {
            ...
            if ($del_res)
                $message = __('Entry deleted successfully', 'ARForms');
    ...
    function include_css_from_form_content($post_content) {
        ...
        global $post, $submit_ajax_page, $arfversion;
        ...
        if (!is_admin()) {
            ...
            if (is_array($newvalarr) && count($newvalarr) > 0) {
                ...
                foreach ($newvalarr as $newkey => $newval) {
                    ...
                    wp_register_style('arfformscss' . $newval, $upload_main_url . '/maincss_' . $newval . '.css', array(), $arfversion);
...
$_POST['vpb_captcha_code'] = isset($_POST['vpb_captcha_code'])?$_POST['vpb_captcha_code']:'';
                    $security_code = trim(strip_tags($_POST['vpb_captcha_code']));
    ...
    if (!strstr($file_path, "post.php")) {
        ...
        <script type="text/javascript">
            ...
            function arp_load_simple_colpicker(){
        ...
        <?php
        if( isset($_COOKIE['arf_radio_checkbox_loaded']) && count( $_COOKIE['arf_radio_checkbox_loaded'][$form_id]) > 0 ){
        ?> load_arf_icheck(); <?php
            unset($_COOKIE['arf_radio_checkbox_loaded'][$form_id]);
        }
        
        if( isset($_COOKIE['arf_selectpicker_loaded'] ) && count( $_COOKIE['arf_selectpicker_loaded'][$form_id]) > 0 ){
        ?> load_arf_selectpicker(); <?php
            unset($_COOKIE['arf_selectpicker_loaded'][$form_id]);
        }
        
        if( isset($_COOKIE['arf_colorpicker_loaded']) && count( $_COOKIE['arf_colorpicker_loaded'][$form_id]) > 0 ){
        ?> arp_load_colorpicker(); <?php
        unset($_COOKIE['arf_colorpicker_loaded'][$form_id]);
        }
        
        if( isset($_COOKIE['arf_basic_colorpicker_loaded']) && count($_COOKIE['arf_basic_colorpicker_loaded'][$form_id]) > 0 ){
         ?> arp_load_simple_colpicker(); <?php
         unset($_COOKIE['arf_basic_colorpicker_loaded'][$form_id]);
        }
