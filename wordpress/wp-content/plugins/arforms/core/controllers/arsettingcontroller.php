...
class arsettingcontroller {
    ...
    function arf_send_test_mail() {
        ...
        $subject = (isset($_POST['subject']) && !empty($_POST['subject'])) ? $_POST['subject'] : __('SMTP Test E-Mail', 'ARForms');
        ...
        echo $arnotifymodel->send_notification_email_user($send_to, $subject, $message, $reply_to, $reply_to_name, '', array(),true,true,true);
    ...
    function addons_page() {
        ?><script type="application/javascript" >jQuery('#arfsaveformloader').show();</script> <?php
        $plugins = get_plugins();
        ...
        $urltopost = 'https://www.arformsplugin.com/arf/addons/addon_list.php';
        ...
        if (is_wp_error($raw_response) || $raw_response['response']['code'] != 200) {
            ...
            ?><script type="application/javascript">jQuery('#arfsaveformloader').hide();</script><?php
        } else {
    ...
    function head() {
        ...
        if (isset($_REQUEST['page']) && $_REQUEST['page'] != '' && $_REQUEST['page'] != "ARForms-settings")
            $js_file = ARFURL . '/js/jquery/jquery-ui-themepicker.js?ver=' . $arfversion;
    ...
    function verify_autores($atts) {
        ...
        if ($name == 'mailchimp') {
            ...
            if (count($lists) > 0) {
                ...
                echo '<div class="sltstandard" style="float:none; display:inline;"><select name="mailchimp_listid" id="mailchimp_listid" style="width:150px;"  data-width="150px">';
