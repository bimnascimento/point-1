...
final class PWBE_SQL_Builder {
	...
	public function get_products( $post ) {
        ...
        if ( $products = PWBE_DB::query( "
			...
			JOIN
				{$wpdb->posts} AS parent ON (parent.ID = post.ID)
			LEFT JOIN
				{$wpdb->posts} AS variation ON (variation.post_parent = post.ID AND variation.post_type = 'product_variation')
			$common_joins
			WHERE
				post.post_type = 'product'
    ...
    private function build_common_sql( $suffix, $group_type, $fields, &$sql_fields, &$sql_joins ) {
		...
		for ($row_index = 0; $row_index < count( $fields['row'] ); $row_index++ ) {
			...
			switch ( $field_name ) {
				...
				case 'post_excerpt':
					$row_sql = $this->string_search( 'post.post_excerpt', $filter_type, $field_value );
				break;
	...
	private function build_order_by( $post ) {
		...
		$order_by .= ', COALESCE(NULLIF(parent_post_id, 0), post_id), NULLIF(parent_post_id, 0) IS NOT NULL, post_id';
