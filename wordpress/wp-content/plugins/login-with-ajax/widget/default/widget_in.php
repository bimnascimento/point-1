				...
				<?php
					...
					if ( $lwa_data['profile_link'] == '1' ) {
						if( function_exists('bp_loggedin_user_link') ){
							...
							?>
							<a href="<?php echo home_url('/minha-conta'); ?>"><?php esc_html_e('Profile','login-with-ajax') ?></a><br/>
							<?php
						}
					}
					//Logout URL
					?>
					<a id="wp-logout" title="Log out" href="<?php echo wp_logout_url( get_permalink() ); ?>"><?php esc_html_e( 'Log Out' ,'login-with-ajax') ?></a><br />
					<?php
					//Blog Admin
