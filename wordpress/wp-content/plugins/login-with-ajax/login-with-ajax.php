...
class LoginWithAjax {
	...
	public static function init(){
		...
		self::$url_login = self::template_link(home_url('/login/'));
		//self::$url_register = self::template_link(self::getRegisterLink());
		self::$url_register = self::template_link(home_url('/cadastro/'));
		//self::$url_remember = self::template_link(site_url('wp-login.php?action=lostpassword', 'login_post'));
		self::$url_remember = self::template_link(home_url('/login/'));
		...
		if( !empty($_REQUEST['log']) && !empty($_REQUEST['pwd']) && trim($_REQUEST['log']) != '' && trim($_REQUEST['pwd'] != '') ){
			...
			if ( strtolower(get_class($loginResult)) == 'wp_user' ) {
				...
				self::$current_user = $loginResult;
						...
						$verificado = get_user_meta( $loginResult->ID, 'wcemailverified', true );
						if ( empty($verificado) ) {

									 if(!session_id()) {
									 		@session_start();
									 }
									 $idUser = $loginResult->ID;
									 wp_logout();
									 if( !session_id() ) {
									 		@session_start();
									 }
									 $_SESSION["verify_email"] = array(
					           "verify_email_user" => $idUser,
					           "tipo"=>'ativacao',
					         );
									 $return['result'] = false;
									 $return['error'] = __('E-mail aguardando validação.');
									 echo 'setTimeout(function (e) { window.location.reload(true); },1000);';
									...
									echo 'MyObject.notice_success("Aguarde, preparando seu usuário!");';
									...
									$user_login_acess = get_user_meta( $loginResult->ID , 'user_login_acess', true );
									$lavanderia = get_user_meta(  $loginResult->ID , 'dokan_store_name', true );
									if( empty($user_login_acess) && !empty($lavanderia) ){
											$return['redirect'] = home_url('/?page=configuracao-lavanderia');
									}
									update_user_meta( $loginResult->ID, 'user_login_acess', ( (int) $user_login_acess) + 1 );
