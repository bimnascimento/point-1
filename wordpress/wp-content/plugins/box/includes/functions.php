...
function dokan_count_posts( $post_type, $user_id ) {
    ...
    if ( false === $counts ) {
        ...
        if( $post_type == 'product' && ! is_admin() ){
          ...
          //dump($not_product_id);
          //exit;
          if( isset($not_product_id ) )
            $query .= " AND ID <> $not_product_id ";
        }
        ...
        foreach ( $results as $row ) {
            ...
            //if( $post_type=='product' && ( $counts['publish'] || $counts['total'] ) ){
                //$counts[ $row['post_status'] ] = (int) $row['num_posts'] - 1 ;
                //$total -- ;
            //}
        }
...
function dokan_author_pageviews( $seller_id ) {
    ...
    if ( $pageview === false ) {
        ...
        $sql = "SELECT SUM(meta_value) as pageview
            ...
            WHERE meta.meta_key = 'pageview'
            AND p.post_author = %d
            AND p.post_name = %d
            AND p.post_status IN ('publish', 'pending', 'draft')";

        $count = $wpdb->get_row( $wpdb->prepare( $sql, $seller_id, $store_slug ) );
...
function dokan_author_total_sales( $seller_id ) {
    ...
    if ( $earnings === false ) {
        ...
        $sql = "SELECT SUM(order_total) as earnings
            ...
            WHERE seller_id = %d AND order_status IN('wc-completed', 'wc-processing', 'wc-on-hold', 'wc-pagamento-ok', 'wc-saiu-para-entrega', 'wc-saiu-para-coleta', 'wc-lavando-pecas')";
...
function dokan_get_seller_bank_details( $seller_id ) {
    ...
    if ( isset( $payment['razao_social'] ) ) {
        $details[] = sprintf( __( 'Razão Social: %s', 'dokan' ), $payment['razao_social'] );
    }
    if ( isset( $payment['cnpj'] ) ) {
        $details[] = sprintf( __( 'CNPJ: %s', 'dokan' ), $payment['cnpj'] );
    }
    if ( isset( $payment['ie'] ) ) {
        $details[] = sprintf( __( 'Inscrição Estadual: %s', 'dokan' ), $payment['ie'] );
    }
...
function dokan_get_seller_address( $seller_id = '', $get_array = false ) {
    ...
    if ( isset( $profile_info['address'] ) ) {
        ...
        $endereco     = isset( $address['endereco'] ) ? $address['endereco'] : '';
        $bairro     = isset( $address['bairro'] ) ? $address['bairro'] : '';
        $numero     = isset( $address['numero'] ) ? $address['numero'] : '';
        $complemento     = isset( $address['complemento'] ) ? $address['complemento'] : '';
