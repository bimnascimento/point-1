 ...
 */
function dokan_seller_sales_statement() {
    $start_date = date( '01-m-Y', current_time('timestamp') );
    $end_date = date( 'd-m-Y', strtotime( 'midnight', current_time( 'timestamp' ) ) );
...
function dokan_get_order_report_data( $args = array(), $start_date, $end_date ) {
    ...
    $query['where']  = "
        ...
        AND     do.order_status IN ('" . implode( "','", apply_filters( 'woocommerce_reports_order_statuses', array( 'wc-completed', 'wc-processing', 'wc-on-hold', 'wc-pagamento-ok', 'wc-saiu-para-coleta', 'wc-saiu-para-entrega', 'wc-lavando-pecas' ) ) ) . "')
        ";
    ...
    if ( $debug || $nocache || ( false === ( $result = get_transient( 'dokan_wc_report_' . $query_hash ) ) ) ) {
        ...
        if ( $filter_range ) {
            if ( $end_date == date('d-m-Y', current_time( 'timestamp' ) ) ) {
...
function dokan_sales_overview_chart_data( $start_date, $end_date, $group_by ) {
    ...
    //$order_commision     = dokan_prepare_chart_data( $orders, 'post_date', 'earning', $chart_interval, $start_date_to_time, $group_by );
    //dump($order_amounts);
...
function dokan_top_sellers() {
    ...
    $order_items = apply_filters( 'woocommerce_reports_top_sellers_order_items', $wpdb->get_results( "
        ...
        AND     posts.post_status   != 'trash'
        AND     do.seller_id = {$current_user->ID}
        AND     do.order_status IN ('" . implode( "','", apply_filters( 'woocommerce_reports_order_statuses', array( 'wc-on-hold', 'wc-completed', 'wc-processing', 'wc-pagamento-ok', 'wc-saiu-para-entrega', 'wc-saiu-para-coleta', 'wc-lavando-pecas' ) ) ) . "')
    ...
    <form method="post" action="" class="report-filter dokan-form-inline dokan-clearfix">
        <div class="dokan-form-group">
            ...
            <input type="text" class="datepicker" name="start_date" id="from" readonly="readonly" value="<?php echo esc_attr( date('d-m-Y', $start_date) ); ?>" />
        </div>

        <div class="dokan-form-group">
            <label for="to"><?php _e( 'To:', 'dokan' ); ?></label>
            <input type="text" class="datepicker" name="end_date" id="to" readonly="readonly" value="<?php echo esc_attr( date('d-m-Y', $end_date) ); ?>" />
        </div>
            ...
            <?php
                ...
                foreach ( $found_products as $product_id => $sales ) {
                    ...
                    if ( $product_title ) {
                        ...
                        $orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=' . urlencode( $product_title ) . '&shop_order_status=' . implode( ",", apply_filters( 'woocommerce_reports_order_statuses', array( 'wc-on-hold', 'wc-completed', 'wc-processing', 'wc-pagamento-ok', 'wc-saiu-para-entrega', 'wc-saiu-para-coleta', 'wc-lavando-pecas' ) ) ) );
                    } else {
                        $product_name = __( 'Product does not exist', 'dokan' );
                        $orders_link = admin_url( 'edit.php?s&post_status=all&post_type=shop_order&action=-1&s=&shop_order_status=' . implode( ",", apply_filters( 'woocommerce_reports_order_statuses', array( 'wc-on-hold', 'wc-completed', 'wc-processing', 'wc-pagamento-ok', 'wc-saiu-para-entrega', 'wc-saiu-para-coleta', 'wc-lavando-pecas' ) ) ) );
                    }
...
function dokan_top_earners() {
    ...
    <table class="table table-striped">
        <thead>
            <tr>
                <th><?php _e( 'Product', 'dokan' ); ?></th>
                <th colspan="2"><?php _e( 'Pedidos', 'dokan' ); ?></th>
            </tr>
        </thead>
        <tbody>
            <?php
