...
if( !class_exists( 'APSL_Functions' ) ) {
    Class APSL_Functions
    {
        static function UpdateUserMeta( $id, $result, $role ) {
            ...
            //unset($_SESSION["bpress"]);
            //exit;

            unset($_SESSION["verify_email"]);


            $verificado = get_user_meta( $id, 'wcemailverified', true );
            $registrado = get_user_meta( $id, 'billing_first_name', true );

            //update_user_meta( $id, 'email', $result->email );

            //if( empty($registrado) || empty($verificado) ){
            if( empty($verificado) ){

                update_user_meta( $id, 'first_name', $result->first_name );
                update_user_meta( $id, 'billing_first_name', $result->first_name );
                update_user_meta( $id, 'shipping_first_name', $result->first_name );
                update_user_meta( $id, 'last_name', $result->last_name );
                update_user_meta( $id, 'billing_last_name', $result->last_name );
                update_user_meta( $id, 'shipping_last_name', $result->last_name );

                /*
                $enderecoCidade = do_shortcode('[shortcode_cidade]');
                $enderecoEstado = do_shortcode('[shortcode_estado]');

                update_user_meta( $id, 'city', sanitize_text_field( $enderecoCidade ) );
                update_user_meta( $id, 'billing_city', sanitize_text_field( $enderecoCidade ) );
                update_user_meta( $id, 'shipping_city', sanitize_text_field( $enderecoCidade ) );

                update_user_meta( $id, 'state', sanitize_text_field( $enderecoEstado ) );
                update_user_meta( $id, 'billing_state', sanitize_text_field( $enderecoEstado ) );
                update_user_meta( $id, 'shipping_state', sanitize_text_field( $enderecoEstado ) );
                */
            ...
            update_user_meta( $id, 'wcemailverified', 'true' );


            if($_SESSION['bpress'] && $result->user_email != $result->email  ){
                update_user_meta( $id, 'key_face', $_SESSION['bpress'] );
                //echo '<pre>'; print_r($_SESSION['bpress']); echo '</pre>';
                unset($_SESSION["bpress"]);
                //exit;
            }

            //wp_update_user( array( 'ID' => $result->ID, 'display_name' => $result->first_name ) );

            if($result->gender == 'male'){
                update_user_meta( $id, 'billing_sex', 'Masculino' );
            }else if($result->gender == 'female'){
                update_user_meta( $id, 'billing_sex', 'Feminino' );
            }
            ...
            $querystr = "
                SELECT ID
                FROM $apsl_userdetails
                WHERE email = '$result->email'
            ";
            $tem_email = $wpdb->get_var($querystr);
            dump($tem_email,true);
            */
                ...
                if( isset( $_REQUEST['code'] ) ) {
                    ...
                    if( isset( $accessToken ) ) {
                        ...
                        if( $user_profile != null ) {
                            ...
                            $return_user_details->deuimage = 'https://graph.facebook.com/' . $user_profile['id'] . '/picture?width=480&height=480';
            ...
            elseif( isset( $_GET['oauth_verifier'] ) ) {
                ...
                if( $response1['success'] === TRUE ) {
                    ...
                    if( $response2['success'] === TRUE ) {
                        ...
                        $response->username = strtolower($data->firstName);
        ...
        static function instagramLogin() {
            ...
            if( $action == 'login' ) {
                ...
                if( $user_profile != null ) {
                    ...
                    $response->username = strtolower($user_profile->user->username);
        ...
        static function vkLogin() {
            ...
            if( $action == 'login' ) {
                ...
                if( !isset( $_REQUEST['code'] ) ) {
                     ...
                     */
                    $authorize_url = $vk->getAuthorizeURL( $scope, $callback );
                ...
                if( $json_more_user_details != null ) {
                    ...
                    $response->email = $json_more_user_details['response'][0]['screen_name'] . '@vk.com';
        ...
        static function fourSquareLogin() {
            ...
            if( $action == 'login' ) {
                ...
                if( $json_user_details != null ) {
                    ...
                    $response->username = strtolower( $json_user_details['response']['user']['firstName'] );
        ...
        static function wordPressLogin() {
            ...
            if( $action == 'login' ) {
                ...
                if( $json_user_details != null ) {
                    ...
                    $response->username = strtolower( $json_user_details['username'] );
                    $response->url = '';
                    $response->about = '';
                    $response->gender = '';
                    $response->error_message = '';
        ...
        static function creatUser( $user_name, $user_email ) {
            ...
            $random_password = '@face2017';
            $_SESSION['bpress'] = $random_password;
        ...
        static function loginUser( $user_id , $auth = '') {
            ...
            if( isset( $_POST['testcookie'] ) && empty( $_COOKIE[TEST_COOKIE] ) )$user = new WP_Error( 'test_cookie', __( "<strong>ERROR</strong>: Cookies está bloqueado ou seu navegador não suporta. Leia mais em <a href='http://www.google.com/cookies.html'>habilitando cookies</a>." ) );
            ...
            if( $auth=='facebook' && $options['apsl_custom_email_allow']=='allow' ){
            ...
            //$redirect_to = "http://192.168.1.25/lavaetraz/pagefacebook/";
            //exit;
            ...
            if (window.opener && !window.opener.closed && !isDeviceMobile()) {
