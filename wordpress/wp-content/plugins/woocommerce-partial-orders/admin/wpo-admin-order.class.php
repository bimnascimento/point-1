...
class WPO_Admin_Order {
	...
	{	
		...
		if( !is_null( $product ) ){
			...
			$this->output_order_item_shipped_values( $item['item_meta'] );
			echo '</td>';
	...
	{	
		...
		if( isset( $item['shipped'] ) && !empty( $item['shipped'][0] ) ){
			...
			if( count($shipped_info > 1) ){ //multiple shippings
				$shipped_date = $shipped_info[ count( $shipped_info ) - 1 ]['date']; //date of last shipped item
			} else {
				$shipped_date = $shipped_info[0]['date'];
			}
			
			if( $quantity_shipped == $item['_qty'][0] ){ //all shipped
	...
	public function set_item_shipped( $order_id, $item_id, $shipped_quantity = 0, $shipped_date = NULL )
	{
		...
		if( $shipped_quantity == 0 ){
			$total_shipped = $this->get_total_quantity_shipped( $shipped_info ); //get remaining quantity
			$shipped_quantity = (int)$item['_qty'][0] - $total_shipped;
		...
		if( $shipped_quantity > 0 ){
			...
			woocommerce_update_order_item_meta( $item_id, 'shipped', $shipped_info );
			...
			$item['shipped'][0] = $shipped_info; //save a db call and update it here
		}
	...
	{
		woocommerce_update_order_item_meta( $item_id, 'shipped', '' );
	...
	public function ajax_unset_item_shipped()
	{
		...
		$this->unset_item_shipped( $order_id, $item_id );
		
		$output = $this->output_order_item_shipped_values( array(), TRUE );
