	...
	{	
		...
		switch ($screen->id) {
			case 'shop_order':
				...
				$allow_not_shipped = get_option( 'woocommerce_partial_orders_mark_not_shipped', 'no' );
				if( $allow_not_shipped == 'yes' ){
					$options[] = '<option value=\"bulk_unset_shipped\">Unset as shipped</option>';
				}
				
				if( version_compare( WC_VERSION, '2.2', '<' ) ){ // Woo 2.1 and below
					wc_enqueue_js("$('#woocommerce-order-items .bulk_actions select').append('<optgroup label=\"Partial Orders\">".implode( '', $options )."</optgroup>'); ");
				} else {
					wc_enqueue_js("$('.wc-order-bulk-actions .bulk-actions select').append('<optgroup label=\"Partial Orders\">".implode( '', $options )."</optgroup>'); ");
				}
