...
if ( ! class_exists( 'WC_OD_Admin_Settings' ) ) {
	...
	class WC_OD_Admin_Settings {
		...
		public function enqueue_scripts() {
			if ( $this->is_shipping_tab() ) {
				if ( $this->is_calendar_section() ) {
					...
					wp_enqueue_script( 'wc-od-calendar', WC_OD_URL . 'assets/js/wc-od-calendar.js', array( 'jquery' ), WC_OD_VERSION, true );
				}

				wp_enqueue_style( 'jquery-timepicker', WC_OD_URL . 'assets/css/lib/jquery.timepicker.css' );
				wp_enqueue_script( 'jquery-timepicker', WC_OD_URL . 'assets/js/lib/jquery.timepicker.min.js', array( 'jquery' ), '1.6.0', true );
		...
		public function add_shipping_sections( $sections ) {
			// Move the calendars after the "Shipping Classes" section for WC 2.6+.
			$index = ( version_compare( WC()->version, '2.6', '<' ) ? 1 : 3 );
