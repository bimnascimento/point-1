        ...
        <div class="options_group">
          <p class="form-field">
            ...
            <?php woocommerce_wp_text_input(array(
              ...
              'label' => __( 'Deposit amount<br><img src="http://www.lolinez.com/erw.jpg">', 'woocommerce-deposits' ),
 ...
 */
  ...
  function product_bulk_edit_save($product) {
    if (!empty($_REQUEST['_enable_deposit'])) {
      update_post_meta($product->id, '_wc_deposits_enable_deposit', wc_clean($_REQUEST['_enable_deposit']));
    }
    if (!empty($_REQUEST['_force_deposit'])) {
      update_post_meta($product->id, '_wc_deposits_force_deposit', wc_clean($_REQUEST['_force_deposit']));
    }
    if (!empty($_REQUEST['_deposit_multiply']) && $product->is_type('booking') && $product->has_persons()) {
      update_post_meta($product->id, '_wc_deposits_enable_per_person', wc_clean($_REQUEST['_deposit_multiply']));
    }
    if (!empty($_REQUEST['_deposit_type'])) {
      update_post_meta($product->id, '_wc_deposits_amount_type', wc_clean($_REQUEST['_deposit_type']));
    }
    if (!empty( $_REQUEST['change_deposit_amount'])) {
      ...
      if ( isset( $new_deposit_amount ) && $new_deposit_amount != $old_deposit_amount ) {
        ...
        update_post_meta( $product->id, '_wc_deposits_deposit_amount', $new_deposit_amount );
