	...
	//jQuery(document).on("click", ".single_add_to_cart_button", function(e) {
		...
		if( form.find('label.tm-error').length > 0  ) {
			...
			form.find('label.tm-error').each(function() {
				if( jQuery( this ).attr("style") === undefined || jQuery( this ).attr("style") === null ){
					if( form.find( '.verifica-obrigatorio' ).length == 0 ){
							...
							MyObject.notice_error("Para continuar, preencha os campos obrigatórios!");
					}
					//console.log("error");
					erros = true;
				}
		...
		jQuery.ajax({
	      ...
	      e.stopPropagation();
				...
				if( jQuery(response).find('.woocommerce-error').length > 0 ){
										...
										texto_notice = jQuery(response).find('.woocommerce-error').wrap('<div>').parent().html();
										MyObject.notice_fatal( "Verifique seu pedido!" );
				...
				// SE SUCESSO
										...
										MyObject.notice_success( "Adicionado!" );
										...
										texto_notice = jQuery(response).find('.woocommerce-message').wrap('<div>').parent().html();
										...
										window.texto2 = jQuery(div_to_insert);
				...
				*/
				...
				//MyObject.notice_info( texto_notice );
				//console.log('FIM');
