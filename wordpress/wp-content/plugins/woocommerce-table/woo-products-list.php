...
function wc_products_list_pro($allsets) {
  ...
  if ($wcplpro_pagination != 'no' && $wcplpro_pagination !== '' ) {
    ...
    if (!empty($pagination_posts_ids)) {
      ...
      ///$qnt = get_post_meta($post_id,'_stock',true);
  ...
  if ( $query->have_posts()) {
    ...
    if (($wcplpro_globalcart == 1 || $wcplpro_globalcart == 2) && ($wcplpro_globalposition == 'top' || $wcplpro_globalposition == 'both')) {
      ...
      $out .= apply_filters('wcplpro_global_btn', '
        <div class="gc_wrap">
          <a data-position="top" href="#globalcart" class="add_to_cart_pedido_peca add_to_cart_button globalcartbtn submit btn single_add_to_cart_button button alt" data-post_id="gc_'.$current_post->ID .'" id="gc_'. $vtrand .'_top" class="btn button alt">'. __('Add selected to cart', 'wcplpro') .'<span class="vt_products_count"></span></a>
    ...
    while ( $query->have_posts() ) {
      ...
      $form = '
		<form action="'. esc_url( $product->add_to_cart_url() ) .'" method="POST" data-product_id="'.  $product->id .'" data-id="wcplpro_product_'. $product->id .'" class="cart wcplpro_product_'. $product->id .' vtajaxform" enctype="multipart/form-data">
			<input type="hidden" name="product_id" value="'. esc_attr( $product->id ) .'" />
			<input type="hidden" name="add-to-cart" value="'. esc_attr( $product->id ) .'" />
			<input type="hidden" name="cpf_product_price" class="cpf-product-price" value="'.wc_format_decimal($product->get_display_price(), 2) .'" />
      ...
      foreach ($wcplpro_order as $colkey => $col_title) {
        ...
        if ($colkey == 'wcplpro_title' && $wcplpro_title == 1) {
          ...
          if ($wcplpro_attributes == 1 && !empty($custom_attributes)){
            ...
            <td class="titlecol"  data-title="'. apply_filters('wcplpro_dl_title', $headenames[$colkey], $product) .'">
              <a href="'. get_permalink($product->id)  .'" target="_blank" title="'. $product->get_title() .'">'. $product->get_title() .'</a>
        ...
        if ($colkey == 'wcplpro_sku' && $wcplpro_sku == 1) {
          $col_checker[$colkey] = true;
          $allcolumns[$colkey] = '<td class="skucol" data-title="'. apply_filters('wcplpro_dl_sku', $headenames[$colkey], $product) .'" >
          <a href="'. get_permalink($product->id)  .'" target="_blank" title="'. $product->get_title() .'">
        ...
        if ($colkey == 'wcplpro_qty' && $wcplpro_qty == 1) {
          ...
          if ($product->is_in_stock() || $product->backorders_allowed()) {
            ...
            $allcolumns[$colkey] .= '
              <input type="number" step="'. apply_filters('wcplpro_qty_step', $wcplpro_qty_step, $product) .'" name="wcplpro_quantity" value="'. ($wcplpro_default_qty != '' ? apply_filters('wcplpro_default_qty', $wcplpro_default_qty, $product) : 1) .'" title="Qty" class="input-text qty text" size="4" min="1" '. (intval($product->get_total_stock()) > 0 ? 'max="'. $product->get_total_stock() .'"': '') .'>
            ';
        ...
        if ($colkey == 'wcplpro_price' && $wcplpro_price == 1) {
          $col_checker[$colkey] = true;
					...
					if ( is_user_logged_in() ) {
							...
							$allcolumns[$colkey] = '
							<td class="pricecol">
								<a href="#" class="click-ver-preco">ver preço</a>
							</td>';
					}
        ...
        if ($colkey == 'wcplpro_cart') { // && $wcplpro_cart == 1
          ...
          //$allcolumns['wcplpro_cart'] = '<td class="cartcol '. ($wcplpro_cart == 0 ? 'wcplprohide' : '') .' '. $woocommerce_add_to_cart_class .'" data-title="">'.$woocommerce_before_add_to_cart_button;
					$allcolumns['wcplpro_cart'] = '<td class="cartcol '. ($wcplpro_cart == 0 ? 'wcplprohide' : '') .' '. $woocommerce_add_to_cart_class .'" data-title="">';
          ...
          if (isset($product_meta['_stock_status']) || $product->backorders_allowed()) {
            ...
            if (
              ...
              $carttext = __('Add to cart', 'wcplpro' );
						...
						//echo '<pre>';
						//var_dump($woocommerce_before_add_to_cart_button);
						//echo '</pre>';
						$post_categories = get_the_ID();
						//$sub_cats = get_categories( $args2 );
						$categorias = $product->get_categories();
						//dump($categorias);
						//exit;

						$pos = strpos($categorias, 'servcliente') !== false;
						//echo $product->id.' - '.( strlen(trim($woocommerce_before_add_to_cart_button)) ).'<br/>';
						$posClass = '';
						if($pos) {
								$posClass = 'opcoes';
								$carttext = __( 'Adicionar', 'wcplpro' );
						}
            ...
            if ($product_type == 'variable' || (!(wc_format_decimal($product->get_display_price(), 2) > 0) && $wcplpro_zero_to_out == 1) || $product_meta['_stock_status'][0] == 'outofstock') {
              $allcolumns['wcplpro_cart'] .= '<a href="'. get_the_permalink() .'" data-id="add2cartbtn_'. $product->id .'" data-product_id="'. $product->id .'" class="add2cartbtn_'. $product->id .' '. implode(' ', $wcplpro_button_classes) .' alt">'. apply_filters('single_add_to_cart_text', $product->add_to_cart_text(), $product->product_type, $product) .'</a>';
              ...
              $allcolumns['wcplpro_cart'] .= $form.'
                <button data-id="add2cartbtn_'. $product->id .'" type="submit" data-product_id="'. $product->id .'" class="add2cartbtn_'. $product->id .' '. implode(' ', $wcplpro_button_classes) .'">'. apply_filters('single_add_to_cart_text', $carttext, $product->product_type, $product) .'</button>';
              if ($wcplpro_ajax == 1 || $wcplpro_globalcart == 1 || $wcplpro_globalcart == 2) {
                $allcolumns['wcplpro_cart'] .= '
                  <div class="added2cartwrap added2cart_'.$product->id.' " data-id="added2cart_'.$product->id.'"><span class="added2cart" >&#10003;</span></div>
                ...
                $allcolumns['wcplpro_cart'] .= '
                  <div class="added2cartwrap notvisible added2cart_'.$product->id.' " data-id="added2cart_'.$product->id.'"></div>
            ...
            $allcolumns['wcplpro_cart'] .= '
              <a href="'. get_the_permalink() .'" data-id="add2cartbtn_'. $product->id .'" data-product_id="'. $product->id .'" class="add2cartbtn_'. $product->id .' add_to_cart_button single_add_to_cart_button button alt ajax add_to_cart">'. apply_filters('single_add_to_cart_text', $product->add_to_cart_text(), $product->product_type, $product) .'</a>
              <div class="added2cartwrap notvisible added2cart_'.$product->id.' " data-id="added2cart_'.$product->id.'"></div>
              <span class="vtspinner vtspinner_'. $product->id .' notvisible"></span>
            ';
    ...
    if (($wcplpro_globalcart == 1 || $wcplpro_globalcart == 2) && ($wcplpro_globalposition == 'bottom' || $wcplpro_globalposition == 'both')) {
      ...
      $out .= apply_filters('wcplpro_global_btn', '
        <div class="gc_wrap">
          <a data-position="bottom" href="#globalcart" class="globalcartbtn submit btn add_to_cart_button single_add_to_cart_button button alt" data-product_id="gc_'.$product->id .'" id="gc_'. $vtrand .'_bottom" class="btn button alt">'. __('Add selected to cart', 'wcplpro') .'<span class="vt_products_count"></span></a>
          <span class="added2cartglobal added2cartglobal_'. $vtrand .'">&#10003;</span>
