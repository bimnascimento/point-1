...
(function( $ ) {
   ...
   $( function() {
        ...
        console.log("pagseguro transparent not mim");
		...
		function pagSeguroGetInstallmentOption( installment ) {
			...
			return '<option '+ ((installment.quantity==1)?'selected':'') + ' value="' + installment.quantity + '" data-installment-value="' + installment.installmentAmount + '">' + installment.quantity + 'x ' + pagSeguroGetPriceText( installment.installmentAmount ) + interestFree + '</option>';
		}
		...
		function pagSeguroInitPaymentForm() {
			...
			// CPF.
      $( '#pagseguro-card-holder-cpf' ).val($( '#billing_cpf' ).val());
			$( '#pagseguro-card-holder-cpf' ).mask( '999.999.999-99', { placeholder: ' ' } );

			// Birth Date.
      $( '#pagseguro-card-holder-birth-date' ).val($( '#billing_birthdate' ).val());
			$( '#pagseguro-card-holder-birth-date' ).mask( '99/99/9999', { placeholder: ' ' } );

			// Phone.
            $( '#pagseguro-card-holder-phone' ).val($( '#billing_phone' ).val());
