...
if ( ! class_exists( 'WPSEO_Local_Import' ) ) {
	...
	class WPSEO_Local_Import extends WPSEO_Local_Import_Export {
		...
		private function handle_csv_import() {
			if ( isset( $_POST['csv-import'] ) && check_admin_referer( 'wpseo_local_import_nonce', 'wpseo_local_import_nonce_field' ) ) {
				...
				if ( ! empty( $_FILES['wpseo'] ) && ! move_uploaded_file( $_FILES['wpseo']['tmp_name']['csvuploadlocations'], $csv_path ) ) {
					$this->messages[] = array(
						'type'    => 'error',
						'content' => __( 'Sorry, there was an error while uploading the CSV file.<br>Please make sure the ' . $this->wpseo_upload_dir . ' directory is writable (chmod 777).', 'yoast-local-seo' ),
					);
				}
						...
						if ( empty( $location['lat'] ) && empty( $location['long'] ) ) {
							...
							if ( ! is_wp_error( $geo_data ) && ! empty( $geo_data->results[0] ) ) {
								...
								if ( $geo_data->get_error_code() == 'wpseo-query-limit' ) {
									$this->messages[] = array(
										'type'    => 'error',
										/* translators: First %s extends to the link opening tag to the Yoast Local SEO admin page; Second %s closes that tag */
										'content' => sprintf( __( 'The usage of the Google Maps API has exceeds their limits. Please consider entering an API key in the %soptions%s', 'yoast-local-seo' ), '<a href="' . admin_url( 'admin.php?page=wpseo_local' ) . '">', '</a>' ),
									);

									if ( ! empty( $last_imported ) ) {
										$this->messages[] = array(
											'type'    => 'warning',
											/* translators: First %s extends to the edit link for the last imported location; Second %s is the anchor text. */
											'content' => sprintf( __( 'The last successfully imported location is <a href="%s">%s</a>', 'yoast-local-seo' ), get_edit_post_link( $last_imported ), get_the_title( $last_imported ) ),
										);
									}
									break;
								}
									...
									$this->messages[] = array(
										'type'    => 'error',
										/* translators: First %s extends to the link opening tag to the location; Second %s closes that tag */
										'content' => sprintf( __( 'Location <em>' . esc_attr( $location['name'] ) . '</em> could not be geo-coded. %sEdit this location%s.', 'yoast-local-seo' ), '<a href="' . admin_url( 'post.php?post=' . esc_attr( $post_id ) . '&action=edit' ) . '">', '</a>' ),
									);
								}
							}
						}
		...
		public function import_html() {
			...
			/* translators: First %s extends to link opening tag; second %s closes the tag */
