...
class woocommerce_confirmation_email {
    ...
    public function __construct() {
        ...
        // CONTEUDO DO EMAIL - LINK DE ATIVACAO
    ...
    function please_confirm_email_message() {
        dump('please_confirm_email_message');
        exit;
    ...
    */
    ...
    function please_login_email_message() {
        dump('please_login_email_message');
        exit;
        ?>
        <ul class="woocommerce-info">
    ...
    // CARREGA NO HEAD DO WORDPRESS - COOKIE
    function authenticate_user_by_email() {

        if (  isset($_GET["confirma_email"]) ) {

            $user_meta = explode("@", base64_decode($_GET["confirma_email"]));
            $user_id = $user_meta[1];
            $user_code = $user_meta[0];

            if ( get_user_meta((int) $user_id, "wcemailverifiedcode", TRUE) == $user_code ) {

                  update_user_meta((int) $user_id, "wcemailverified", "true");
                  update_user_meta((int) $user_id, "wcemailverifiedcode", "ok");
            ...
            }else if (  get_user_meta((int) $user_id, "wcemailverifiedcode", TRUE) == 'ok' ) {

                  $_SESSION["verify_email"] = array(
                    "verify_email_user" => $user_id,
                    "tipo"=>'verificado-true',
                  );

            }

        }
        if (  isset($_GET["ativacao_reenviar"])  ) {

            //$_SESSION["reenviado"] = 'ok';
            //$_SESSION["verify_email"] = array("verify_email_user" => $user_ID);
            //exit;
            $this->reenviado = true;
            $user_id = base64_decode($_GET["ativacao_reenviar"]);
            $user_id = explode("-", base64_decode($_GET["ativacao_reenviar"]));
            $user_id = $user_id[1];
    ...
    public function getcokkies_on_vrification() {
        dump('getcokkies_on_vrification');
        exit;
    ...
    // ADMIN - AUTORIZA EMAIL
    public function manual_verify_user() {
        //dump('manual_verify_user');
        //exit;
...
// VERIFICA SE WOOCOMMERCE INSTALADO - MENSAEM
...
// VERIFICA SE WOOCOMMERCE INSTALADO
