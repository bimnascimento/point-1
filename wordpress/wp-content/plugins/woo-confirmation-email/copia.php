...
class woocommerce_confirmation_email {
    ...
    public function __construct() {
        ...
        //add_filter( 'woocommerce_registration_message', 'filter_woocommerce_registration_message', 10, 1 );
        //add_filter('please_confirm_email_message');

        add_action("admin_head", array($this, "manual_verify_user"));
        add_action("wp", array($this, "authenticate_user_by_email"));
    }

    public function filter_woocommerce_registration_message( $var ) {
    ...
    public function codeMailSender($email) {
        ...
        $preMesaage = "<html><body><div style='max-width:700px;padding:5px;margin:auto;font-size:14px;line-height:18px'>" . apply_filters('the_content', $message) . "<div style='clear:both'></div></div></body></html>";
        //global $email
        ...
        //$mailResult = false;
        //$mailResult = wp_mail($email, $Email_title, $preMesaage, $header);
        //dump( $mailResult );
        //exit;
    ...
    public function new_user_registeration($customer, $user_id) {
        ...
        $mensagem = '<strong>Email de confirma&ccedil;&atilde;o</strong>: Por favor, verifique seu email com link para ativa&ccedil;&atilde;o. <a style="font-size:14px;color:red;" href="'.$link.'"> Clique aqui, para reenviar email de confirma&ccedil;&atilde;o.</a>';
        wc_add_notice($mensagem, "notice");*/

        /*$errors = new WP_Error();
    	  $errors->add( 'password_reset_mismatch', 'asdasdas' );
        wc_add_notice( 'teste22', 'error');
        apply_filters( 'woocommerce_add_message', 'sdfsdfds' );
        */

        echo do_shortcode('[shortcode_valida_email]');
        ...
        //wp_loginout('http://192.168.1.25/lavaetraz/novo-cadastro', true );
        //wp_login_url(get_permalink());
        //wp_redirect(get_permalink());
        //wp_logout_url('http://192.168.1.25/lavaetraz/novo-cadastro/');
        //exit;
        //wp_redirect(get_permalink());
        //wp_redirect(get_the_permalink($this->my_account));
        //wp_redirect("index.php");
        //wc_add_notice( 'teste333', 'error');
        //apply_filters( 'woocommerce_add_message', 'sdfsdfds' );
    ...
    function please_confirm_email_message() {
        ...
        <ul style="display: inline-block;">
            <li><strong>Email de confirma&ccedil;&atilde;o</strong>: Por favor, verifique seu email com link para ativa&ccedil;&atilde;o. <a style='font-size:14px;color:red 'href="<?php echo $link ?>"> Clique aqui, para reenviar email de confirma&ccedil;&atilde;o.</a></li>
        </ul>
    ...
    function authenticate_user_by_email() {
        ...
        if (isset($_GET["woo_confirmation_verify"])) {
            ...
            if (get_user_meta((int) $user_meta[1], "wcemailverifiedcode", TRUE) == $user_meta[0]) {
                ...
                wc_clear_notices();
                wc_add_notice("Sua conta foi verificada, por favor efetue seu login.", "notice");
                //wc_clear_notices();
    ...
    public function wc_email_verification_code() {
        ...
        $hyperlink = add_query_arg(array("woo_confirmation_verify" => base64_encode($createLink)), home_url('/login') );
        //$hyperlink = add_query_arg(array("woo_confirmation_verify" => base64_encode($createLink)), get_the_permalink($this->login) );


        //    $hyperlink = get_the_permalink($this->my_account) . "?woo_confirmation_verify=" . base64_encode($createLink);
        $link = "<a href='" . $hyperlink . "'> Clique aqui para verificar </a>";
        return $link;
