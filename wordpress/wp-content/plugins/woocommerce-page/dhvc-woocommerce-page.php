...
if(!defined('DHVC_WOO_PAGE_VERSION'))
	define('DHVC_WOO_PAGE_VERSION','1.8.9');
...
class DHVC_Woo_Page{
	...
	public function init(){
		...
		vc_add_shortcode_param ( 'dhvc_woo_product_page_field_products_ajax', 'dhvc_woo_product_page_setting_field_products_ajax');
	...
	public function wc_get_template_part($template, $slug, $name){
		...
		if($slug === 'content' && $name === 'single-product'){
			...
			if($dhvc_woo_page_product = get_post_meta(get_the_ID(),'dhvc_woo_page_product',true)):
			...
			//dump($product_template_id); return;
