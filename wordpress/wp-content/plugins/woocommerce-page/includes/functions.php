...
function get_the_product_page_content( $more_link_text = null, $strip_teaser = false){
	...
	if ( count( $content ) > 1 ) {
		if ( $more ) {
			$output .= '<span id="more-' . get_the_ID() . '"></span>' . $content[1];
		} else {
			if ( ! empty( $more_link_text ) )
				$output .= apply_filters( 'the_content_more_link', ' <a href="' . get_permalink() . "#more-{get_the_ID()}\" class=\"more-link\">$more_link_text</a>", $more_link_text );
