<?php
/*
  Plugin Name: Dokan Seller Verification
  ...
  License: GPL2
  Text Domain: dokan-verification
 */
...
if ( is_admin() && file_exists( dirname( __FILE__ ) . '/lib/plugin-updater/wedevs-updater.php' ) ) {
    require_once dirname( __FILE__ ) . '/lib/plugin-updater/wedevs-updater.php';

    new WeDevs_Plugin_Update_Checker( plugin_basename( __FILE__ ), 'dokan-verification' );
}
...
class Dokan_Seller_Verification {
    ...
    function load_verfication_admin_template() {
        add_submenu_page( 'dokan', __( 'Seller Verifications', 'dokan-verification' ), __( 'Seller Verifications' ), 'manage_options', 'dokan-seller-verifications', array( $this, 'seller_verfications_page' ) );
    }
     ...
     */
    function need_dokan(){
        $error = sprintf( __( '<b>Dokan Seller Verification</b> requires %sDokan plugin%s to be installed & activated!' , 'dokan-verification' ), '<a target="_blank" href="https://wedevs.com/products/plugins/dokan/">', '</a>' );
    ...
    public function register_dashboard_menu( $urls ) {
        ...
        $urls['verification'] = array(
            'title' => __( 'Verification', 'dokan-verification' ),
    ...
    public function dokan_update_verify_info() {
        ...
        if ( !wp_verify_nonce( $postdata['dokan_verify_action_nonce'], 'dokan_verify_action' ) ) {
            wp_send_json_error( __( 'Are you cheating?', 'dokan-verification' ) );
        }
        ...
        if ( isset( $postdata['dokan_v_id_type'] ) && isset( $postdata['dokan_gravatar'] ) ) {
            ...
            $msg = sprintf( __( 'Your ID verification request is Sent and %s approval', 'dokan-verification' ), $seller_profile['dokan_verification']['info']['dokan_v_id_status'] );
    ...
    public function dokan_id_verification_cancel() {
        ...
        $msg = __( 'Your ID Verification request is cancelled', 'dokan-verification' );
    ...
    public function dokan_address_verification_cancel() {
        ...
        $msg = __( 'Your Address Verification request is cancelled', 'dokan-verification' );
    ...
    public function dokan_sv_form_action() {
        ...
        $msg = __( 'Information updated', 'dokan-verification' );
        wp_send_json_success( $msg );
    }
    ...
    public function dokan_update_verify_info_insert_address() {
        ...
        $msg = __('Please fill all the required fields', 'dokan-verification' );
        ...
        $msg = __( 'Your Address verification request is Sent and Pending approval', 'dokan-verification' );
    ...
    public function dokan_v_send_sms() {
        ...
        $sms  = WeDevs_SMS_Gateways::instance();
