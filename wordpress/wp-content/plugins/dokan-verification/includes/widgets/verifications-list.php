...
class Dokan_Verification_list extends WP_Widget {
    ...
    public function __construct() {
        $widget_ops = array( 'classname' => 'dokan-verification-list', 'description' => __( 'Dokan Seller Verifications', 'dokan-verification' ) );
        parent::__construct( 'dokan-verification-list', __( 'Dokan: Verification', 'dokan-verification' ), $widget_ops );
    }
    ...
    function form( $instance ) {
        $instance = wp_parse_args( (array) $instance, array(
            'title' => __( 'ID Verification', 'dokan-verification' ),
        ) );

        $title = $instance['title'];
        ?>
        <p>
            <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'dokan-verification' ); ?></label>
    ...
    function print_info_items( $key, $item ) {
        ...
        if ( isset( $item['dokan_v_id_status'] ) ) {
            if ( $item['dokan_v_id_status'] === 'approved' ) {
                ?>
                <li class="clearfix">
                    <i class="fa fa-user"></i><span><?php _e( 'Photo ID', 'dokan-verification' ); ?></span><i class="fa fa-check-circle-o verification-icon dokan-right"></i>
                </li>
            <?php
            }
        }
        ...
        if ( isset( $item['store_address'] ) ) {
            ...
            if ( sizeof( $this->verify_address( $item['store_address'] ) ) == 0 ) {
                ?>
                <li class="clearfix">
                    <i class="fa fa-map-marker"></i><span><?php _e( 'Postal Address', 'dokan-verification' ); ?></span><i class="fa fa-check-circle-o verification-icon dokan-right"></i>
                </li>
            <?php
            }
        }
        ...
        if ( isset( $item['phone_status'] ) ) {
            if ( $item['phone_status'] === 'verified' ) {
                ?>
                <li class="clearfix">
                    <i class="fa fa-phone-square"></i><span><?php _e( 'Phone', 'dokan-verification' ); ?></span><i class="fa fa-check-circle-o verification-icon dokan-right"></i>
                </li>
            <?php
            }
        }
...
function show_verification_widget() {
    if ( !is_active_sidebar( 'sidebar-store' ) ) {
        ...
        the_widget( 'Dokan_Verification_list', array( 'title' => __( 'Verifications', 'dokan-verification' ) ), $args );
    }
}