(function($){
    ...
    //console.log(jQuery(".loading-site").css("display"));
    ...
    $('.argmc-wrapper form').bind('jqv.field.result', function (event, field, errorFound, prompText) {
        //console.log("jqv.field.result");
        var formGroup = field.parents('.form-row').first();
        if (errorFound) {
            jQuery('.checkout').removeClass('validating');
        ...
        //$(".loading-site").hide();
        //jQuery('.checkout').removeClass('validating');
        jQuery(".argmc-nav .argmc-next").prop('disabled', false);
        jQuery("#argmc-submit").prop('disabled', false);
    });

    //Validate forms
    jQuery('.argmc-wrapper').find('form').validationEngine({
       maxErrorsPerField : 1,
	     prettySelect: true,
	     usePrefix: 's2id_'
    });
	...
	*/
    ...
    function navigateTo(index) {
        ...
        $('.argmc-wrapper').trigger('argmcBeforeStepChange', [prevStep, nextStep]);
            ...
            //$(".loading-site").hide();
            //console.log("argmcBeforeStepChange");
    ...
    $('.argmc-nav .argmc-next').on('click', function() {
        //$(".loading-site").show();
        jQuery('.checkout').addClass('validating');
        jQuery(".argmc-nav .argmc-next").prop('disabled', true);
        setTimeout(function() {
    ...
    $('#argmc-submit').on('click', function() {
        //jQuery("#argmc-submit").prop('disabled', true);
        //$(".loading-site").show();
        //jQuery('.checkout').addClass('validating');
        //$(".blockUI").hide();
        //setTimeout(function() {
            if ( jQuery('.checkout').validationEngine('validate') === true ) {
                ...
                //jQuery('.checkout').addClass('validating');
                //MyObject.loadingSite();
                //jQuery("#place_order").trigger("click", function(e) {
                   //$(".loading-site").show();
                    //console.log("ok3");
                    //e.preventDefault();
                    //jQuery('.checkout').addClass('validating');
  ...
  //  });
   //*/
    ...
    //$('.woocommerce-message .close-button, .woocommerce-error .close-button,.woocommerce-info .close-button').on('click', function(){
