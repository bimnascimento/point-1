...
class class_uv_emails_templates  {
	...
	public function uv_templates_editor(){
			...
			foreach($templates_data as $key=>$templates){
				...
				$html.= '<label>'.__('Enable ?', UV_TEXTDOMAIN).'<br/>';	// .options			
				...
				$html.= '<label>'.__('Email To: (Copy)', UV_TEXTDOMAIN).'<br/>';	// .options				
				...
				$html.= '<label>'.__('Email from name:', UV_TEXTDOMAIN).'<br/>';	// .options				
				...
				$html.= '<label>'.__('Email from:', UV_TEXTDOMAIN).'<br/>';	// .options				
				...
				$html.= '<label>'.__('Email Subject:',UV_TEXTDOMAIN).'<br/>';	// .options				
				...
				$html.= '<br/><label>'.__('Email Body:',UV_TEXTDOMAIN).'<br/>';	// .options				
	...
	public function uv_templates_settings_display(){
		...
		if(empty($_POST['uv_hidden'])) {
			...
			if($_POST['uv_hidden'] == 'Y'){
				...
				$html.= '<div class="updated"><p><strong>'.__('Changes Saved.', UV_TEXTDOMAIN ).'</strong></p></div>';	
			}
		}
		
		
		$html.= '<div class="wrap">';
		$html.= '<div id="icon-tools" class="icon32"><br></div><h2>'.__(UV_PLUGIN_NAME.' - Emails Templates', UV_TEXTDOMAIN).'</h2>';	
		...
		$html.= '<p class="submit">
                    <input class="button button-primary" type="submit" name="Submit" value="'.__('Save Changes',UV_TEXTDOMAIN ).'" />
                    <input class="reset-email-templates button" type="button" value="'.__('Reset',UV_TEXTDOMAIN ).'" />
					</p>';
		$html.= '</form>';
