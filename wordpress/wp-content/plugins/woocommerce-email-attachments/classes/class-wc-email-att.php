...
//		add_action( 'woocommerce_init', array( $this, 'handler_wc_init'), 100 );
		...
		add_action( 'woocommerce_email_footer', array( $this, 'handler_wc_email_attachment_footer' ), 900 );
	}
...
//		register_taxonomy_for_object_type('category', 'my_post_type');
	 ...
	 * @return string|array									(see documentation of wp_mail) 
	 */
	public function handler_wc_email_attachments( $attachment = '', $emailsubject = '', $object = null )
	{
		if( empty($emailsubject ) || ! is_string( $emailsubject ) )
		{
			return $attachment;
	 ...
	 */
	public function handler_wc_email_attachment_footer() 
	{
		$footers = array();		
		$note = array();
		...
		foreach ( $footers as &$footer ) 
		{
			...
			if( empty( $head ) && empty( $text ) )  {  continue;  }
	 ...
	 * @return string|array										(see documentation of wp_mail) 
	 */
	protected function add_attachments( $attachment = '', $att_option = '', $emailsubject = '', $object = null )
	{
		...
		{
			if( $this->attachment_infos[ $attachment_id]['error'] )  {  continue;  }
