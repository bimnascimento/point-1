    ...
    // we go through all parameters and theck if they are valid.
     foreach ($atts as $key => $option) {
        if ($key == 'id') {
            ...
            if ($id_new != $option) {
               $error_array[] = 'Id is not valid. Please check the documentation.';                
            }
        }
...
if ($show_iframe_as_layer == 'true' || $show_iframe_as_layer == 'external') {
   ...
   if ($ios_scroll) {
     ...
     if (empty($show_iframe_as_layer_header_file)) {
       $layer_div_style = "-webkit-overflow-scrolling: touch; overflow: auto;display:none;position:fixed;z-index:100002;".$layer_div_base. ";margin:0px !important;padding:0px !important;";
     } else {
       $layer_div_style = "display:none;position:fixed;z-index:100002;".$layer_div_base.";margin:0px !important;padding:0px !important;";
     ...
     $width='100%';
     $height='100%';
     $style .= ";display:none;margin:0px !important;padding:0px !important;position:fixed;z-index:100002;" . $layer_div_base;
   }
   $src="about:blank";
   $show_part_of_iframe = 'false';
   $enable_lazy_load = 'false';
