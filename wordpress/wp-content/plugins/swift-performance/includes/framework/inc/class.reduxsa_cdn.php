<?php
    ...
    /**
     * ReduxSA Framework CDN Container Class
     *
            ...
            private static function _cdn( $register = true, $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
                ...
                if ( $cdn_is_up ) {
                    ...
                    if ( is_wp_error( $cdn_response ) || wp_remote_retrieve_response_code( $cdn_response ) != '200' ) {
                        if ( class_exists( 'ReduxSA_VendorURL' ) ) {
                            $src = ReduxSA_VendorURL::get_url( $handle );
                            ...
                            if ( ! self::is_enqueued( $handle, 'enqueued', $is_script ) ) {
                                $msg = __( 'Please wait a few minutes, then try refreshing the page. Unable to load some remotely hosted scripts.', 'reduxsa-framework' );
                                if ( self::$_parent->args['dev_mode'] ) {
                                    $msg = sprintf( __( 'If you are developing offline, please download and install the <a href="%s" target="_blank">ReduxSA Vendor Support</a> plugin/extension to bypass the our CDN and avoid this warning', 'reduxsa-framework' ), 'https://github.com/reduxsaframework/reduxsa-vendor-support' );
                                }

                                self::$_parent->admin_notices[] = array(
                                    'type'    => 'error',
                                    'msg'     => '<strong>' . __( 'ReduxSA Framework Warning', 'reduxsa-framework' ) . '</strong><br/>' . sprintf( __( '%s CDN unavailable.  Some controls may not render properly.', 'reduxsa-framework' ), $handle ) . '  ' . $msg,
                                    'id'      => $handle . $tran_key,
                                    'dismiss' => false,
                                );
            ...
            private static function _vendor_plugin( $register = true, $handle, $src_cdn, $deps, $ver, $footer_or_media, $is_script = true ) {
                if ( class_exists( 'ReduxSA_VendorURL' ) ) {
                    ...
                    if ( ! self::$_set ) {
                        self::$_parent->admin_notices[] = array(
                            'type'    => 'error',
                            'msg'     => sprintf( __( 'The <a href="%s">Vendor Support plugin</a> (or extension) is either not installed or not activated and thus, some controls may not render properly.  Please ensure that it is installed and <a href="%s">activated</a>', 'reduxsa-framework' ), 'https://github.com/reduxsaframework/reduxsa-vendor-support', admin_url( 'plugins.php' ) ),
                            'id'      => $handle . '23',
                            'dismiss' => false,
                        );
