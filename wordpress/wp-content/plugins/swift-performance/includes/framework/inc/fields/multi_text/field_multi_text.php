...
// Don't duplicate me!
if ( ! class_exists( 'ReduxSAFramework_multi_text' ) ) {

    /**
     * Main ReduxSAFramework_multi_text class
     *
     * @since       1.0.0
     */
    class ReduxSAFramework_multi_text {
        ...
        public function render() {
            ...
            $this->add_text   = ( isset( $this->field['add_text'] ) ) ? $this->field['add_text'] : __( 'Add More', 'reduxsa-framework' );
            $this->show_empty = ( isset( $this->field['show_empty'] ) ) ? $this->field['show_empty'] : true;

            echo '<ul id="' . $this->field['id'] . '-ul" class="reduxsa-multi-text">';
            ...
            if ( isset( $this->value ) && is_array( $this->value ) ) {
                foreach ( $this->value as $k => $value ) {
                    if ( $value != '' || ($value == '' && $this->show_empty == true) ) {
                        ...
                        echo     '<a' . ' data-id="' . $this->field['id'] . '-ul" href="javascript:void(0);" class="deletion reduxsa-multi-text-remove">' . __( 'Remove', 'reduxsa-framework' ) . '</a>';
                        echo '</li>';
                    }
                }
            ...
            echo '<a href="javascript:void(0);" class="button button-primary reduxsa-multi-text-add" data-add_number="' . $this->field['add_number'] . '" data-id="' . $this->field['id'] . '-ul" data-name="' . $this->field['name'] . $this->field['name_suffix'] . '">' . $this->add_text . '</a><br/>';
        }
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                'reduxsa-field-multi-text-js',
                ReduxSAFramework::$_url . 'inc/fields/multi_text/field_multi_text' . ReduxSA_Functions::isMin() . '.js',
                array( 'jquery', 'reduxsa-js' ),
                time(),
                true
            );
            ...
            if ($this->parent->args['dev_mode']) {
                wp_enqueue_style(
                    'reduxsa-field-multi-text-css',
                    ReduxSAFramework::$_url . 'inc/fields/multi_text/field_multi_text.css',
                    array(),
                    time(),
                    'all'
                );
