<?php
    ...
    if ( ! class_exists( 'reduxsaCoreEnqueue' ) ) {
        class reduxsaCoreEnqueue {
            public $parent = null;
            ...
            public function init() {
                $this->min = ReduxSA_Functions::isMin();

                $this->timestamp = ReduxSAFramework::$_version;
                ...
                add_filter("reduxsa/{$this->parent->args['opt_name']}/localize", array('ReduxSA_Helpers', 'localize'));

                $this->set_localized_data();
                
                /**
                 * action 'reduxsa-enqueue-{opt_name}'
                 *
                 * @deprecated
                 *
                 * @param  object $this ReduxSAFramework
                 */
                do_action( "reduxsa-enqueue-{$this->parent->args['opt_name']}", $this->parent ); // REMOVE

                /**
                 * action 'reduxsa/page/{opt_name}/enqueue'
                 */
                do_action( "reduxsa/page/{$this->parent->args['opt_name']}/enqueue" );
            }
            ...
            private function register_styles() {
                ...
                wp_enqueue_style(
                    'reduxsa-admin-css',
                    ReduxSAFramework::$_url . 'assets/css/reduxsa-admin.css',
                    array(),
                    $this->timestamp,
                    'all'
                );
                ...
                if ( ! $this->parent->args['dev_mode'] ) {
                    wp_enqueue_style(
                        'reduxsa-fields-css',
                        ReduxSAFramework::$_url . 'assets/css/reduxsa-fields.css',
                        array(),
                        $this->timestamp,
                        'all'
                    );
                ...
                // Spectrum CSS
                //*****************************************************************
                //$css_file = 'reduxsa-spectrum.min.css';
                //if ($this->parent->args['dev_mode']) {
                    $css_file = 'reduxsa-spectrum.css';
                //}
                
                wp_register_style(
                    'reduxsa-spectrum-css',
                    ReduxSAFramework::$_url . 'assets/css/vendor/spectrum/' . $css_file,
                    array(),
                    '1.3.3',
                    'all'
                );
                ...
                //*****************************************************************
                wp_enqueue_style(
                    'reduxsa-elusive-icon',
                    ReduxSAFramework::$_url . 'assets/css/vendor/elusive-icons/elusive-icons.css',
                    array(),
                    $this->timestamp,
                    'all'
                );
                ...
                wp_enqueue_style(
                    'qtip-css',
                    ReduxSAFramework::$_url . 'assets/css/vendor/qtip/' . $css_file,
                    array(),
                    '2.2.0',
                    'all'
                );
                ...
                wp_enqueue_style(
                    'jquery-ui-css',
                    apply_filters( "reduxsa/page/{$this->parent->args['opt_name']}/enqueue/jquery-ui-css", ReduxSAFramework::$_url . 'assets/css/vendor/jquery-ui-bootstrap/jquery-ui-1.10.0.custom.css' ),
                    array(),
                    $this->timestamp,
                    'all'
                );
                ...
                if ( $this->parent->args['dev_mode'] ) {
                    ...
                    //*****************************************************************
                    wp_register_style(
                        'reduxsa-color-picker-css',
                        ReduxSAFramework::$_url . 'assets/css/color-picker/color-picker.css',
                    ...
                    //*****************************************************************
                    wp_enqueue_style(
                        'reduxsa-field-media-css',
                        ReduxSAFramework::$_url . 'assets/css/media/media.css',
                        array(),
                        time(),
                        'all'
                    );
                ...
                if ( is_rtl() ) {
                    wp_enqueue_style(
                        'reduxsa-rtl-css',
                        ReduxSAFramework::$_url . 'assets/css/rtl.css',
                        array( 'reduxsa-admin-css' ),
                        $this->timestamp,
                        'all'
                    );
                }
            ...
            private function register_scripts() {
                ...
                //*****************************************************************
                wp_register_script(
                    'reduxsa-select2-sortable-js',
                    ReduxSAFramework::$_url . 'assets/js/vendor/reduxsa.select2.sortable' . $this->min . '.js',
                    array( 'jquery' ),
                    $this->timestamp,
                    true
                );
                ...
                ReduxSA_CDN::register_script(
                    'select2-js',
                    '//cdn.jsdelivr.net/select2/3.5.2/select2' . $this->min . '.js',
                    array( 'jquery', 'reduxsa-select2-sortable-js' ),
                    '3.5.2',
                    true
                );
                ...
                wp_enqueue_script(
                    'qtip-js',
                    ReduxSAFramework::$_url . 'assets/js/vendor/qtip/' . $js_file,
                    array( 'jquery' ),
                    '2.2.0',
                    true
                );
                ...
                // Spectrum JS
                //*****************************************************************
                $js_file = 'reduxsa-spectrum.min.js';
                if ($this->parent->args['dev_mode']) {
                    $js_file = 'reduxsa-spectrum.js';
                }
                
                wp_register_script(
                    'reduxsa-spectrum-js',
                    ReduxSAFramework::$_url . 'assets/js/vendor/spectrum/' . $js_file,
                    array( 'jquery' ),
                    '1.3.3',
                    true
                );                
                ...
                wp_register_script(
                    'reduxsa-vendor',
                    ReduxSAFramework::$_url . 'assets/js/vendor.min.js',
                    array( 'jquery' ),
                    $this->timestamp,
                    true
                );

                array_push( $depArray, 'reduxsa-vendor' );
                ...
                wp_register_script(
                    'reduxsa-js',
                    ReduxSAFramework::$_url . 'assets/js/reduxsa' . $this->min . '.js',
                    $depArray,
                    $this->timestamp,
                    true
                );
            ...
            public function _enqueue_field($field) {
                ...
                if ( isset( $field['type'] ) && $field['type'] != 'callback' ) {
                    ...
                    $field_class = 'ReduxSAFramework_' . $field['type'];

                    /**
                     * Field class file
                     * filter 'reduxsa/{opt_name}/field/class/{field.type}
                     *
                     * @param       string        field class file path
                     * @param array $field        field config data
                     */
                    $class_file = apply_filters( "reduxsa/{$this->parent->args['opt_name']}/field/class/{$field['type']}", ReduxSAFramework::$_dir . "inc/fields/{$field['type']}/field_{$field['type']}.php", $field );
                    if ( $class_file ) {
                        ...
                        if ( ( method_exists( $field_class, 'enqueue' ) ) || method_exists( $field_class, 'localize' ) ) {
                            ...
                            if ( ! wp_script_is( 'reduxsa-field-' . $field['type'] . '-js', 'enqueued' ) && class_exists( $field_class ) && method_exists( $field_class, 'enqueue' ) ) {
                                $theField->enqueue();
                            }
            ...
            private function set_localized_data() {
                ...
                /**
                 * Save pending string
                 * filter 'reduxsa/{opt_name}/localize/save_pending
                 *
                 * @param       string        save_pending string
                 */
                $save_pending = apply_filters( "reduxsa/{$this->parent->args['opt_name']}/localize/save_pending", __( 'You have changes that are not saved. Would you like to save them now?', 'reduxsa-framework' ) );

                /**
                 * Reset all string
                 * filter 'reduxsa/{opt_name}/localize/reset
                 *
                 * @param       string        reset all string
                 */
                $reset_all = apply_filters( "reduxsa/{$this->parent->args['opt_name']}/localize/reset", __( 'Are you sure? Resetting will lose all custom values.', 'reduxsa-framework' ) );

                /**
                 * Reset section string
                 * filter 'reduxsa/{opt_name}/localize/reset_section
                 *
                 * @param       string        reset section string
                 */
                $reset_section = apply_filters( "reduxsa/{$this->parent->args['opt_name']}/localize/reset_section", __( 'Are you sure? Resetting will lose all custom values in this section.', 'reduxsa-framework' ) );

                /**
                 * Preset confirm string
                 * filter 'reduxsa/{opt_name}/localize/preset
                 *
                 * @param       string        preset confirm string
                 */
                $preset_confirm = apply_filters( "reduxsa/{$this->parent->args['opt_name']}/localize/preset", __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'reduxsa-framework' ) );
                global $pagenow;
                $this->parent->localize_data['args'] = array(
                    ...
                    'please_wait'           => __( 'Please Wait', 'reduxsa-framework' ),
                ...
                $this->parent->localize_data['ajax'] = array(
                    'console' => __( 'There was an error saving. Here is the result of your action:', 'reduxsa-framework' ),
                    'alert'   => __( 'There was a problem with your action. Please try again or reload the page.', 'reduxsa-framework' ),
                );

                $this->parent->localize_data = apply_filters( "reduxsa/{$this->parent->args['opt_name']}/localize", $this->parent->localize_data );
                ...
                wp_enqueue_script( 'reduxsa-js' ); // Enque the JS now
