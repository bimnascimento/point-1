(function ( $ ) {
	...
	function tm_init_epo( main_product, is_quickview, product_id, epo_id ) {
				...
				form_submit_events[ form_submit_events.length ] = {
					...
					"on_true": function () {
						...
						var epos = $( epo_selector + '.tm-product-id-' + product_id + '[data-epo-id="' + epo_id + '"]' ).tm_clone(),
							// hidden fields see totals.php
							//epos_hidden = $('.tc-totals-form.tm-product-id-'+product_id+'[data-epo-id="'+epo_id+'"]').tm_clone(),
		...
		function goto_error_item( item ) {
			...
			if ( el ) {
				...
				if ( elsection.is( '.section_popup' ) ) {
					...
					if ( elcpf_hide_element.length > 0 ) {
						//console.log(elcpf_hide_element);
						var elemntoErro = elcpf_hide_element;
						var offsetErro = jQuery(elemntoErro).offset();
						var is_in_viewport = MyObject.isElementInViewport(	elcpf_hide_element	);
		...
		function tm_epo_init( $cart_container, $composite_cart ) {
			...
			// We are forced to use the following trigger as the plugin doesn't offer it own
		...
		function bto_support() {
			...
			main_product.find( '.bto_form,.composite_form' )
				...
				.on( 'woocommerce-product-addons-update.cpf cpf_bto_review', function () {
					var bto_form = $( this );
					//widget_composite_summary_content = $('.widget_composite_summary_content_'+bto_form.data("product_id"));
					...
					bto_form.parent().find( composite_selector ).each( function () {
						...
						//html2       = widget_composite_summary_content.find('.summary_element.summary_element_'+item_id);
		...
		/*function wc_epo_maybe_trigger_slide_position_reset($form, data){
		 var $product             = $form.closest( '.product' ),
		 $product_gallery     = $product.find( '.images' ),
		 reset_slide_position = false,
		 new_image_id = ( data && data.image_id ) ? data.image_id : '';
			...
			if ( $( img ).length > 0 ) {
				...
				main_product
					...
					.on( 'tm_change_product_image', function ( evt, event_data ) {
						// src, element, main_product, epo_holder
						var el = event_data.element,
							el_current = event_data.element_current;
						if ( el && el_current) {
							var //im = el.data("image"),
								imp = el.data( "imagep" ),
	...
	_document.ready( function () {
		...
		$.ajaxPrefilter( function ( options, originalOptions, jqXHR ) {
			...
			if ( originalOptions.data ) {
				...
				if( originalOptions.url.indexOf('tm_cart_item_key') === -1 ) return;

				console.log( originalOptions.url );
				//console.log( originalOptions.url.indexOf( '?' ) + 1 );
				//console.log( originalOptions.url.slice( originalOptions.url.indexOf( '?' ) + 1 ).tmparseParams() );

				//console.log(originalOptions.url);
				//return;
				//var existe = originalOptions.url.indexOf( '?' ) != -1; // true
				//if(!existe) return;

				//var _urldata = new Array;
				if ( originalOptions.url.indexOf('?') !== -1 ) {
						var _urldata = originalOptions.url.slice( originalOptions.url.indexOf( '?' ) + 1 ).tmparseParams();
						console.log("TM-EPO DATA");
				}
				...
				if ( "quantity" in _data && (_data[ "product_id" ] || _data[ "add-to-cart" ] || _urldata[ "product_id" ] || _urldata[ "add-to-cart" ] || _data[ "tcaddtocart" ] ) ) {
					...
					if ( epos.length == 1 ) {
						...
						options.data = options.data + '&' + $.param(
								$.extend(
									//typeof originalOptions.data=="string" ?originalOptions.data.tmparseParams(true):originalOptions.data,
									epos.tm_aserializeObject(),
									obj
								), false );
					}
