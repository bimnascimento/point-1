        ...
        <?php
            if(empty($item_meta['tm_has_dpd']) && empty($item_meta['_tm_has_dpd'])){
                if ($epo['quantity'] <=0){
                    ...
                    .'</div>';
                    if ($epo_edit_cost)
                    echo '<div class="edit" style="display: none;"><input type="'.$input_type.'" name="tm_epo['.$item_id.']['.$key.'][price]" placeholder="0" value="'.( tc_get_price_excluding_tax($_product,array('qty'=>1,'price'=>$epo['price']))/$epo['quantity'] ).'" data-qty="'.( tc_get_price_excluding_tax($_product,array('qty'=>1,'price'=>$epo['price']))/$epo['quantity']  ).'" size="4" class="price" /></div>';
                }
            }else{
