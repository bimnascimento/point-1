...
final class TM_Extra_Product_Options {
	...
	public function tm_get_price_html( $price = '', $product = FALSE ) {
		...
		if ( $type == 'variable' || $type == 'variable-subscription' ) {
			...
			$min = floatval( apply_filters( 'wc_epo_options_min_price', isset( $this->product_minmax[ $id ]['tc_min_variable'][ key( $prices['price'] ) ] ) ? $this->product_minmax[ $id ]['tc_min_variable'][ key( $prices['price'] ) ] : 0, $product, $price ) );
			...
			$max = floatval( apply_filters( 'wc_epo_options_max_price', isset( $this->product_minmax[ $id ]['tc_max_variable'][ key( $prices['price'] ) ] ) ? $this->product_minmax[ $id ]['tc_max_variable'][ key( $prices['price'] ) ] : 0, $product, $price ) );
	...
	public function tm_woocommerce_cart_item_name( $title = "", $cart_item = array(), $cart_item_key = "" ) {
		...
		if ( apply_filters( 'wc_epo_override_edit_options', TRUE, $title, $cart_item, $cart_item_key ) ) {
			...
			//dump( '3.1' );
			//dump( !(is_cart() || is_checkout()) );
			//dump( isset( $cart_item['composite_item'] ) );
			//dump( isset( $cart_item['composite_data'] ) );

			//if ( !(is_cart() || is_checkout()) || isset( $cart_item['composite_item'] ) || isset( $cart_item['composite_data'] ) ) {
			if ( isset( $cart_item['composite_item'] ) || isset( $cart_item['composite_data'] ) ) {
				return $title;
			}
	...
	public function calculate_price( $post_data = NULL, $element, $key, $attribute, $per_product_pricing, $cpf_product_price = FALSE, $variation_id, $price_default_value = 0, $currency = FALSE, $current_currency = FALSE, $price_per_currencies = NULL ) {
		...
		if ( $per_product_pricing ) {
			...
			$_price = wc_format_decimal( floatval( $_price ), FALSE, TRUE );
	...
	public function custom_frontend_scripts() {
		...
		$args = array(
			...
			"tm_epo_global_validator_messages" => array(
				"required"  => (!empty( $this->tm_epo_this_field_is_required_text )) ? $this->tm_epo_this_field_is_required_text : __( "Este campo é obrigatório.", 'woocommerce-tm-extra-product-options' ),
										...
										foreach ( $disabled_dates as $key => $value ) {
											...
											switch ( $format ) {
												...
												case "4":
													...
													//$value[2] = 'c';
													//$value = $value[0];
													//dump(isset($value[2]) && isset($value[1]));
													if( isset($value[2]) && isset($value[1]) ){
														$value = $value[2] . "-" . $value[1] . "-" . $value[0];
	...
	public function tm_woocommerce_product_single_add_to_cart_text() {
		...
		return __( 'Atualizar Pedido', 'woocommerce' );
