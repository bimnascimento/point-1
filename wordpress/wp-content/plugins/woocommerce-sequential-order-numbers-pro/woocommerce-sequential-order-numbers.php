...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.6.0', __( 'WooCommerce Sequential Order Numbers Pro', 'woocommerce-sequential-order-numbers-pro' ), __FILE__, 'init_woocommerce_sequential_order_numbers_pro', array(
...
class WC_Seq_Order_Number_Pro extends SV_WC_Plugin {
	...
	public function __construct() {
		...
		parent::__construct(
			self::PLUGIN_ID,
			self::VERSION,
			array( 'text_domain' => 'woocommerce-sequential-order-numbers-pro' )
		);
	...
	public function find_order_by_order_number( $order_number ) {
		...
		if ( '' !== SV_WC_Order_Compatibility::get_meta( $order, 'order_number_formatted' ) ) {
	 ...
	 *
	 * TODO: Update the get_post_meta call here in the future to use a core meta getter {BR 2017-04-17}
	 *
	 * @since 1.0.0
	 *
	...
	private function generate_sequential_order_number( $post_id, $order_number_meta_name, $order_number_start, $order_number_prefix = '', $order_number_suffix = '', $order_number_length = 1 ) {
		...
		for ( $i = 0; $i < 3 && ! $success; $i++ ) {
			...
			if ( null !== $order_number ) {
				...
				if ( $this->is_performance_mode_enabled() ) {
					update_post_meta( $post_id, $order_number_meta_name, $order_number );
				}
	...
	public function get_order_number( $order_number, $order ) {
		...
		$order_number_formatted = get_post_meta( SV_WC_Order_Compatibility::get_prop( $order, 'id' ), '_order_number_formatted', true );
	...
	private function format_order_number( $order_number, $order_number_prefix = '', $order_number_suffix = '', $order_number_length = 1 ) {
		...
		return str_ireplace( array_keys( $replacements ), $replacements, $formatted );
	}
