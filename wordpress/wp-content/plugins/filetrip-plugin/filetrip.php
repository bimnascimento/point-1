 ...
 * Description: Filetrip is a professional plugin that will let you draw digital files to your websites, and to review, approve and ship it gracefully to (Dropbox, FTP, & Google Drive).  Find me under Settings → Filetrip Settings
 * Version:     2.0.0
 * Author:      Abdulrhman Elbuni
 * Author URI:  http://www.itechflare.com/
 * Text Domain: filetrip-uploader
...
define('ITECHFILETRIPPLUGINURL',plugins_url( '',__FILE__));
require_once( plugin_dir_path( __FILE__ ).'extensions/classes/class-filetrip-constants.php' );

// Generate security key for backup
...
define( 'FILETRIP_SECURE_KEY', md5( serialize( $key ) ) );

// Top dependencies
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/settings-api/class.settings-api.php';
require_once ITECHFILETRIPPLGUINURI . 'includes/cmb2/filetrip-meta.php';
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/functions.php';

// Filetrip Interfaces
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/interfaces/filetrip-channel-interface.php';

// Filetrip Core Classes
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/class-filetrip-settings.php';
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/class-filetrip-upload-list-table.php';
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/admin-notice-helper/admin-notice-helper.php';
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/class-filetrip-channel-utility.php';
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/class-filetrip-uploader-recorder.php';
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/class-filetrip-upload-distributer.php';

// iTechFlare core extension's platform loader
require_once ITECHFILETRIPPLGUINURI . 'extensions/itechflare-core/itechflare.php';

// Advance meta boxes
require_once ITECHFILETRIPPLGUINURI . 'filetrip-activate.php';
...
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/arfaly-backup/class-backdrop.php';
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/arfaly-backup/class-services.php';
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/arfaly-backup/class-email.php';
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/arfaly-backup/class-notices.php';
require_once ITECHFILETRIPPLGUINURI . 'includes/arfaly-lib/arfaly-backup/class-requirements.php';
...
require_once ITECHFILETRIPPLGUINURI . 'extensions/classes/class-filetrip-type-registrations.php';
...
register_post_status( Filetrip_Constants::POST_STATUS, array(
...
if ( empty( $path ) || ( ( ! is_dir( $path ) && ! wp_is_writable( dirname( $path ) ) ) || ( is_dir( $path ) && ! wp_is_writable( $path ) ) ) && false === strpos( $path, $upload_dir['basedir'] ) ) {
  $upload_dir = wp_upload_dir();
...
class Filetrip_Uploader
{
  ...
  static public $settings_slug = Filetrip_Constants::POST_TYPE.'_settings';
  static public $backup_settings_slug = Filetrip_Constants::POST_TYPE.'_backup_setting';
  static public $mime_settings_slug = Filetrip_Constants::POST_TYPE.'_mime_setting';

  private $manage_permissions;
  private $post_type = 'filetrip';
  static public $g_post_type = 'filetrip';
  static public $plugin_upload_page = Filetrip_Constants::MAIN_MENU_PARENT_SLUG.'&page=filetrip_manage_list';
  static public $plugin_option_page = Filetrip_Constants::MAIN_MENU_PARENT_SLUG.'&page=filetrip_settings';
  ...
  public function arfaly_init()
  {
    ...
    $files = glob(Filetrip_Constants::TMP_FOLDER_PATH.DIRECTORY_SEPARATOR.'*');
    ...
    load_plugin_textdomain( 'filetrip-plugin', false, ITECHFILETRIPPLGUINURI . '/languages/' );
  ...
  function filetrip_admin_notice_handler()
  {
    // Skip if empty
  ...
  function filetrip_error_handling($exp)
  {
  ...
  function arfaly_admin_enqueue() {
    ...
    if( $typenow == $this->post_type || (isset($_GET['page']) && $_GET['page'] == 'filetrip_settings') ) {
        wp_enqueue_style( 'filetrip_meta_box_styles', plugin_dir_url( __FILE__ ) . 'assets/css/arfaly-admin.css' );

        // Register the script first.
        wp_register_script( 'meta_box_filetrip_js', plugin_dir_url( __FILE__ ) . 'assets/js/arfaly-admin.js' );
  ...
  function arfaly_func( $atts ) {
    ...
    return Filetrip_Uploader::building_arfaly_container($id);
  }
  ...
  public function load_arfaly_libraries() {
    ...
    wp_enqueue_script(
            ...
            plugins_url( 'assets/js/filetrip-multi-min.js' , __FILE__ ), array('jquery'), Filetrip_Constants::VERSION, true
        );
    wp_enqueue_style( 'closify-default',
            plugins_url( 'assets/css/style.css' , __FILE__ ), array(), Filetrip_Constants::VERSION);
  }
  ...
  public function _notify_admin(FILETRIP_BKP_Scheduled_Backup $schedule = null ) {
      ...
      reset($bkp_file_path);
        ...
        $body .= '<b>To moderate, visit</b>: ['.admin_url(Filetrip_Uploader::$plugin_option_page).']<br>';
  ...
  function itech_submit_arfaly()
  {
      ...
      if(isset($_FILES["SelectedFile"]))
        ...
        $json['data'] = sprintf(__("Invalid upload format.", ARFALY_TEXT_DOMAIN));
        $this->arfaly_report_error($json['data']);
      }
      ...
      if(!$pass_extension_test) $this->arfaly_report_error("Unsupported file type (".$extension.")!");
      ...
      if($chunk_enabled == "true")
      {
        ...
        if(isset($_POST['title']))
          $att_title = sanitize_text_field($_POST['title']);

        if(isset($_POST['desc']))
          $att_description = sanitize_text_field($_POST['desc']);

        $this->handle_chunk_upload($maxFileSize, $post_id);
        return;
      }
      ...
      if($this->settings['auto_approve_user_files'] == 'on')
      {
        ...
        do_action('itf/filetrip/upload/forware/me', $att_id, $att_title, $att_description);
  ...
  function handle_chunk_upload($maxFileSize, $post_id, $att_title='', $att_desc='')
  {
    ...
    $json = array();
    $DestinationDirectory	= Filetrip_Constants::TMP_FOLDER_PATH.DIRECTORY_SEPARATOR;
    $fileName = '';
    ...
    $filePath2 = apply_filters('itf/filetrip/upload/rename', $filePath2, $post_id, $_POST['fileIndx'] );
        ...
        {
          $this->approve_wordpress_attachment($att_id);
          do_action('itf/filetrip/upload/forware/me', $att_id, $att_title, $att_desc);
  ...
  */
  function arfaly_add_file_to_media_uploader( $post_id, $filename, $description, $title) {
      ...
      $file = Filetrip_Constants::TMP_FOLDER_PATH . DIRECTORY_SEPARATOR . $filename;
  ...
  function arfaly_render_menu(){
	...
	function approve_media() {
      ...
      if(isset($_GET['id']))
      {
        ...
        $query_s = $query_s.'source='.Filetrip_Constants::TRANSFER_REQUEST_SOURCE['forward'].'&';
  ...
  function ajax_immediate_backup()
  {
    ...
    echo '<b>Your backup has been processed</b>';
    die();
  }
