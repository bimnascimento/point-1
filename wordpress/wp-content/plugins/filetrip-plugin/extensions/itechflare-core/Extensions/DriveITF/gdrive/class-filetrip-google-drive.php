...
require_once __DIR__ . '/_setting_page.php';
  ...
  class Filetrip_Google_Drive
  {
    ...
    public static function build_client_object($setting)
    {
      ...
      $client->setClientId($setting[Filetrip_Google_Drive::$google_drive_client_id_slug]);
      $client->setClientSecret($setting[Filetrip_Google_Drive::$google_drive_client_secret_slug]);
      $client->setRedirectUri($setting[Filetrip_Google_Drive::$google_redirect_url_slug]);
            ...
            <ul class="arfaly-file-listing">
              ...
              if( $first_file!='' && !empty($first_file['modelData']['parents'])){ ?>
                <li class="arfaly-drive-get-back">
                  <a href="javascript:arfaly_drive_get_folder_list('<?php echo $first_file['modelData']['parents'][0]['id']; ?>')">(Back) ../</a>
                </li>
                <?php 
              }
              foreach( $files as $file ) : ?>
                    <?php if ( $file['mimeType'] == 'application/vnd.google-apps.folder' ) : ?>
                        <li class="arfaly-folder-drive">
                          <a href="javascript:arfaly_update_drive_folder('<?php echo '('.$file['title'].') - '.$file['id']; ?>', '<?php echo $switch; ?>')" class="button">Select</a>
                            <?php echo '<img src="'.$file['iconLink'].'">'; ?>
                          <a href="javascript:arfaly_drive_get_folder_list('<?php echo $file['id']; ?>')"><?php echo $file['title'] ;?></a>
                        </li>
                    <?php endif; ?>
                <?php endforeach; ?>
            </ul>
