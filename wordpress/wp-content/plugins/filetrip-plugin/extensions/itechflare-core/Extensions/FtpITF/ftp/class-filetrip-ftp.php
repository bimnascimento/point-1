...
require_once __DIR__ . '/_setting_page.php';
  ...
  class Filetrip_FTP
  {
    ...
    public function __construct($setting) {
      ...
      // Initialize settings
      $this->ftp_host = $setting[Filetrip_FTP::$ftp_host_slug];
    ...
    public static function resumable_file_upload($destination_path , $file_uri, $sse_enabled = true)
    {
      ...
      try{
        ...
        // Initialize settings
        $ftp_host = $setting[Filetrip_FTP::$ftp_host_slug];
              ...
              <?php echo _e( 'No files in this folder', 'arfaly_plugin' ); ?>
                    <ul class="arfaly-file-listing">
                      <li>
                        ...
                        <a href="javascript:arfaly_ftp_get_folder_list('<?php echo ".."; ?>')">../</a>
                      </li>
                    </ul>
              <?php else : ?>
              <ul class="arfaly-file-listing">
                <?php if($curPath != '/'){ ?>
                  ...
                  foreach( $files as $path => $file ) : ?>
                          <li class="arfaly-folder">
                            <a href="javascript:arfaly_update_dropbox_folder('<?php echo $curPath.$path; ?>', '<?php echo $switch; ?>')" class="button">Select</a>
                              <?php 
                                echo $folder_icon;
                              ?>
                            <a href="javascript:arfaly_ftp_get_folder_list('<?php echo $curPath.$path; ?>')"><?php echo basename( $path ) ;?></a>
                          </li>
                  <?php endforeach; ?>
              </ul>
          <?php endif; ?>
