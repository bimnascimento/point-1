...
require_once __DIR__ . '/_setting_page.php';
  ...
  class Filetrip_Dropbox
  {
    ...
    public function __construct($setting) {
      ...
      $this->app_key = $setting[Filetrip_Dropbox::$dropbox_key_slug];
      $this->app_secret = $setting[Filetrip_Dropbox::$dropbox_secret_slug];   
    ...
    *      and progress information should be sent back to the client side to notify users
    ...
    public static function resumable_file_upload($destination_path , $file_uri, $filesize, $userTaggedFolder = false, $username = 'Guest', $sse_enabled = true)
    {
      ...
      }catch(DropboxITF\Exception $exp)
      {
        error_log("Filetrip: " . $exp);
        $shortExpMsg = substr($exp, 0, Filetrip_Constants::ERROR_MESSAGE_MAX_LENGHT);
    ...
    public function get_folder_list_html()
    {
      ...
      $folder_icon = '<img src="'.ITECHFILETRIPPLUGINURL.'/assets/img/dropbox-folder.png" width="30" height="30">';
      ob_start();
      ?>
              ...
              <ul class="arfaly-file-listing">
                <?php if($path != ''){ ?>
                  <li>
                    ...
                    <a href="javascript:arfaly_dropbox_get_folder_list('<?php echo $step_back_path; ?>')">../</a>
                  </li>
                  <?php }
                  foreach( $files['entries'] as $file ) : ?>
                      <?php if ( $file['.tag'] == 'folder' ) : ?>
                          <li class="arfaly-folder">
                            <a href="javascript:arfaly_update_dropbox_folder('<?php echo $file['path_lower']; ?>', '<?php echo $switch; ?>')" class="button">Select</a>
                              <?php echo $folder_icon;?>
                            <a href="javascript:arfaly_dropbox_get_folder_list('<?php echo $file['path_lower']; ?>')"><?php echo $file['name'] ;?></a>
                          </li>
                      <?php endif; ?>
                  <?php endforeach; ?>
    ...
    function return_dummy_folder_list()
    {
      ...
      ?>
        ...
        <div class="arfaly-dp-folder-loading spinner"></div>
            ...
            <ul class="arfaly-file-listing">
              <li class="arfaly-folder">
                <a href="javascript:arfaly_update_dropbox_folder('/Assets', 'filetrip_settings\\[dropbox_folder\\]')" class="button">Select</a>
                <img src="http://localhost/freelance/wp-content/plugins/filetrip-plugin/assets/img/dropbox-folder.png" width="30" height="30">
                <a href="javascript:arfaly_dropbox_get_folder_list('/itechflare Assets')">Assets</a>
              </li>
              <li class="arfaly-folder">
                <a href="javascript:arfaly_update_dropbox_folder('/Multimedia', 'filetrip_settings\\[dropbox_folder\\]')" class="button">Select</a>
                <img src="http://localhost/freelance/wp-content/plugins/filetrip-plugin/assets/img/dropbox-folder.png" width="30" height="30">
                <a href="javascript:arfaly_dropbox_get_folder_list('/Multimedia')">Multimedia</a>
              </li>
