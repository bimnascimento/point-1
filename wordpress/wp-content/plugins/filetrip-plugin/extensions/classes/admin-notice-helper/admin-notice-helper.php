...
if ( ! class_exists( 'Filetrip_Admin_Notice_Helper' ) ) {

	class Filetrip_Admin_Notice_Helper {
		...
		public static function get_singleton() {
			if ( ! isset( self::$instance ) ) {
				self::$instance = new Filetrip_Admin_Notice_Helper();
			}
		...
		public function enqueue( $message, $type = 'update' ) {
			...
			$this->notices[ $type ][]   = (string) apply_filters( 'filetrip_enqueue_message', $message );
		...
		public function shutdown() {
			if ( $this->notices_were_updated ) {
				update_option( 'filetrip_notices', $this->notices );
			}
		}
	} // end Filetrip_Admin_Notice_Helper

	Filetrip_Admin_Notice_Helper::get_singleton(); // Create the instance immediately to make sure hook callbacks are registered in time

	if ( ! function_exists( 'filetrip_add_notice' ) ) {
		function filetrip_add_notice( $message, $type = 'update' ) {
			Filetrip_Admin_Notice_Helper::get_singleton()->enqueue( $message, $type );
		}
	}
}
