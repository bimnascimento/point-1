...
function arfaly_get_icon($imgName, $name='', $size='30', $link = '', $title = '')
{
  ...
  return $hrefStart.'<img title="'.$title.'" src="'.ITECHFILETRIPPLUGINURL.'/assets/img/'.$imgName.'.png" width="'.$size.'" height="'.$size.'">'.$hrefEnd.$name;
}
    ...
    $meta_boxes['new_arfaly_metabox'] = array(
		...
		'fields'     => array(
            ...
            array(
				...
				'desc'    => __( 'Allow only these extensions (Use comma seperator). Leave it empty to allow WP default mime list. <br><a href="'.admin_url( Filetrip_Uploader::$plugin_option_page ).'">To customize mime</a>', 'filetrip-plugin' ),
