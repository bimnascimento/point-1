...
class ITSEC_Recaptcha {
	...
	public function comment_form_field_comment( $comment_field ) {
		...
		$comment_field .= $this->get_recaptcha( 'comment' );
	...
	public function comment_form_submit_button( $submit_button ) {
		...
		$submit_button = $this->get_recaptcha( 'comment' ) . $submit_button;
	...
	public function login_form() {
		...
		$this->show_recaptcha( 'login', true );
	...
	public function register_form() {
		...
		$this->show_recaptcha( 'register', true );
	...
	public function show_recaptcha( $type, $add_margins = false ) {
		echo $this->get_recaptcha( $type, $add_margins );
	}

	public function get_recaptcha( $type, $add_margins = false ) {
		...
		if ( 'invisible' === $this->settings['type'] ) {
			...
			if ( $add_margins ) {
				$style_value = 'margin:10px 0';
			} else {
				$style_value = 'margin:0';
			}
	...
	public function validate_captcha() {
		if ( empty( $_POST['g-recaptcha-response'] ) ) {
			...
			return new WP_Error( 'itsec-recaptcha-form-not-submitted', esc_html__( 'You must verify you are a human.', 'it-l10n-ithemes-security-pro' ) );
		}
		...
		return new WP_Error( 'itsec-recaptcha-incorrect', esc_html__( 'The captcha response you submitted does not appear to be valid. Please try again.', 'it-l10n-ithemes-security-pro' ) );
