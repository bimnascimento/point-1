...
class WCCF_Pricing
{
                ...
                'advanced_fees' => array(
                    ...
                    'children'  => array(
                        ...
                        'fee_per_character' => array(
                            'label'         => __('Fee x Character count', 'rp_wccf'),
                            'context'       => array('product_field', 'product_prop', 'checkout_field'),
                            'is_options'    => false,
                        ),
                        ...
                        // Fee x Field value
                        'fee_x_value' => array(
                            'label'         => __('Fee x Field value', 'rp_wccf'),
                            'context'       => array('product_field', 'product_prop', 'checkout_field'),
                            'is_options'    => false,
                        ),
                    ),
     ...
     * @return float
     */
    public static function get_adjusted_price($price, $product_id, $variation_id = null, $posted = array(), $quantity = 1, $calculate_taxes = false, $rounded = true)
    {
        ...
        if (!WCCF_WC_Product::skip_product_fields($product_id, $variation_id)) {
            $adjusted_price = self::get_price_adjusted_by_fields($adjusted_price, $product_id, $variation_id, 'product_field', $posted, $quantity, $calculate_taxes, $rounded);
        }

        // Adjust price by product properties
        $adjusted_price = self::get_price_adjusted_by_fields($adjusted_price, $product_id, $variation_id, 'product_prop', array(), 1, false, $rounded);
     ...
     * @return float
     */
    private static function get_price_adjusted_by_fields($price, $product_id, $variation_id, $context, $posted = array(), $quantity = 1, $calculate_taxes = false, $rounded = true)
    {
        ...
        for ($i = 0; $i < $quantity; $i++) {
            ...
            foreach ($fields as $field) {
                ...
                if ($context === 'product_field') {
                    $value = self::get_product_field_value_for_price_adjustment($field, $product_id, $posted, $i);
                }
                else if ($context === 'product_prop') {
                    $value = self::get_product_property_value_for_price_adjustment($field, $product_id);
                }
     ...
     * @return mixed
     */
    private static function get_product_field_value_for_price_adjustment($field, $product_id, $posted, $quantity_index = null)
    {
        ...
        if (WCCF_Settings::get('product_field_prices_include_default')) {
     ...
     * @return mixed
     */
    private static function get_product_property_value_for_price_adjustment($field, $product_id)
    {
        // Check if default values should be considered
    ...
    public static function adjust_price($original_price, $price, $product_id, $pricing_method, $pricing_value, $field_value, $calculate_taxes)
    {
        ...
        $tax_adjusted_pricing_value = $calculate_taxes ? $product->get_display_price($pricing_value) : $pricing_value;
    ...
    public static function get_adjustment_value($original_price, $price_to_adjust, $pricing_method, $pricing_value, $field_value)
    {
        ...
        switch ($pricing_method) {
            ...
            case 'fees_fee':
                return (float) $pricing_value;

            case 'fees_percentage_fee':
                return (float) ($original_price * $pricing_value / 100);

            case 'fees_compound_percentage_fee':
                return (float) ($price_to_adjust * $pricing_value / 100);

            case 'advanced_fees_fee_per_character':
                return (float) ($pricing_value * strlen(trim((string) $field_value)));

            case 'advanced_fees_fee_x_value':
                return (float) ($pricing_value * abs((float) $field_value));
