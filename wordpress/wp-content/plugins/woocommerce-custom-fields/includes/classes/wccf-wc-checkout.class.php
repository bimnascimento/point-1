...
class WCCF_WC_Checkout
{
    ...
    public function add_order_item_meta($item_id, $values)
    {
        ...
        if (!empty($values['wccf'])) {
            ...
            foreach ($values['wccf'] as $field_id => $field_value) {
                ...
                // Store value
                $field->store_value($item_id, $field_value, $quantity_index);
            }
        }
    }
    ...
    public function add_order_meta($order_id, $posted)
    {
        ...
        WCCF_Field_Controller::store_field_values($order_id, 'checkout_field', false, true);

        // Store posted user field values in both user meta (if logged in) and order meta
        WCCF_Field_Controller::store_field_values($order_id, 'user_field', false, true);
    }
     ...
     *
     * @access public
     * @param bool $include_not_taxable
     * @return array
     */
    public static function get_wc_tax_class_list($include_not_taxable = true)
    {
        $tax_classes = array();
        ...
        if (wc_tax_enabled()) {
            ...
            // Add Not Taxable
            $tax_classes['wccf_not_taxable'] = __('Not Taxable', 'rp_wccf');
