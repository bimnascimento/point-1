...
class WCCF_Migration
{
    ...
    public function file_download_from_1()
    {
        ...
        if (is_user_logged_in() && $_GET['wccf_version_1_file_download'] === 'wccf') {
            ...
            // post_id is order item id
                ...
                // Check if user owns order
                if (!empty($_GET['post_id']) && RightPress_Helper::user_owns_wc_order(get_current_user_id(), $order_id)) {
                    $access_granted = true;
                }
            }
        }
        ...
        if ($_GET['wccf_version_1_file_download'] === 'wccf') {
            ...
            $fields = get_post_meta($_GET['post_id'], '_' . $_GET['wccf_version_1_file_download'], true);
        }
     ...
     * @return mixed
     */
    public static function get_stored_value_from_1($item_id, $field)
    {
        $meta_key_map = array(
        ...
        $values = get_post_meta($item_id, $meta_access_key, true);
                ...
                if ($field->field_type_is('file')) {
                    ...
                    // Move files to new location
                        ...
                        if ($storage_data = WCCF_Files::store_file(array('tmp_name' => $value['file']['path']))) {
                            ...
                            update_post_meta($item_id, $meta_access_key, $values);
                            ...
                            // Store file data
                            update_post_meta($item_id, WCCF_Field::get_file_data_access_key($access_key), $file_data);
                        }
                    }
                }
