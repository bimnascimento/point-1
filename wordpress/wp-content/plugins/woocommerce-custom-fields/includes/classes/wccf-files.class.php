...
class WCCF_Files
{
     ...
     * @access public
     * @param string $access_key
     * @param array $meta_contexts
     * @param int $item_id
     * @return void
     */
    public static function delete_by_access_key($access_key, $meta_contexts = null, $item_id = null)
    {
        // Check if current user has access to file
        if (!WCCF_Files::current_user_can_access($access_key)) {
            return;
        ...
        if ($item_id !== null) {
            ...
            RightPress_Helper::delete_meta($meta_access_key, $meta_contexts, $item_id);
        }
        ...
        try {
            ...
            if (!empty($_POST['item_id'])) {
                ...
                if ($field->context_is('user_field')) {
                    update_user_meta($_POST['item_id'], $meta_access_key, $file_data);
                }
                // All other field data stored in product meta or order meta
                // Note - order item files submitted by admin are temporarily stored under order meta, not order item meta
                else {
                    update_post_meta($_POST['item_id'], $meta_access_key, $file_data);
                }
            }
            else {
                ...
                // Prevent WooCommerce from overwriting our changes
