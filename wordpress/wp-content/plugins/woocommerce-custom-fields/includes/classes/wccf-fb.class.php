...
class WCCF_FB
{
    ...
    public static function checkbox_or_radio($type, $params, $field = null)
    {
        ...
        if (!empty($params['options'])) {
            ...
            foreach ($params['options'] as $option_key => $label) {
                ...
                $field_html .= '<li ' . $user_field_styles . ' ><input type="' . $type . '" value="' . $option_key . '" ' . $checked . ' ' . $attributes . '>' . (!empty($label) ? ' ' . $label : '') . $price_html . '</li>';
            }
     ...
     * @return bool
     */
    public static function validate_select($value, $options)
    {
        return self::validate_select_or_radio($value, $options);
    }
     ...
     * @return bool
     */
    public static function validate_radio($value, $options)
    {
        return self::validate_select_or_radio($value, $options);
    }
     ...
     * @return bool
     */
    public static function validate_select_or_radio($value, $options)
    {
        // Value must be a string and must validate against predefined options
        if (gettype($value) !== 'string' || !self::validate_value_against_options($value, $options)) {
     ...
     * @return bool
     */
    public static function validate_multiselect($value, $options)
    {
        return self::validate_multiselect_or_checkbox($value, $options);
    }
     ...
     * @return bool
     */
    public static function validate_checkbox($value, $options)
    {
        return self::validate_multiselect_or_checkbox($value, $options);
    }
     ...
     * @return bool
     */
    public static function validate_multiselect_or_checkbox($value, $options)
    {
        // Cast value to array
        $values = (array) $value;
        ...
        foreach ($values as $value) {
            ...
            if (!self::validate_value_against_options($value, $options)) {
                $validation_passed = false;
                break;
            }
        }
     ...
     * @return bool
     */
    public static function validate_file($value)
    {
        // Get file extension
