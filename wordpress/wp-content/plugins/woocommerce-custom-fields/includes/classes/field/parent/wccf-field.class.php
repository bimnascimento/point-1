...
class WCCF_Field extends WCCF_Post_Object
{
     ...
     * @return mixed
     */
    public function get_stored_value($item_id, $quantity_index = null)
    {
        ...
        if ($this->validate_stored_entry($item_id, $quantity_index)) {
            ...
            // Get stored value
            $stored_value = $this->get_stored_value_from_meta($item_id, $value_access_key);
        ...
        if (WCCF_Migration::support_for('1')) {
            ...
            $stored_value = WCCF_Migration::get_stored_value_from_1($item_id, $this);
    ...
    public function get_stored_extra_data($item_id, $quantity_index = null)
    {
        ...
        $stored_data = $this->get_stored_value_from_meta($item_id, $data_access_key);
    ...
    /**
     * Get stored field value from meta
     *
     * @access public
     * @param int $post_id
     * @param string $access_key
     * @return mixed
     */
    public function get_stored_value_from_meta($post_id, $access_key)
    {
        // Check if such meta key exists
        if (!RightPress_Helper::post_meta_key_exists($post_id, $access_key)) {
            return null;
        }

        // Get and return value from meta
        return get_post_meta($post_id, $access_key, true);
    }
     ...
     * @return mixed
     */
    public function get_final_value($item_id)
    {
        // Get stored value
        $stored_value = $this->get_stored_value($item_id);
     ...
     * @return bool
     */
    public function validate_stored_entry($item_id, $quantity_index = null)
    {
        // Get field id access key
        $field_id_access_key = $this->get_id_access_key($quantity_index);

        // Get stored field id
        $stored_id = $this->get_stored_value_from_meta($item_id, $field_id_access_key);
     ...
     * @return void
     */
    public function store_value($item_id, $value, $quantity_index = null)
    {
            ...
            if (!empty($stored_value) && is_array($stored_value)) {
                ...
                if ($delete_all_files || $delete_removed_files) {
                    ...
                    foreach ($stored_value as $access_key) {
                        ...
                        if ($delete_current_file) {
                            $meta_context = WCCF_Field_Controller::get_meta_context($this->get_context());
                            WCCF_Files::delete_by_access_key($access_key, $meta_context, $item_id);
                        }
                    }
                }
            }
        ...
        /*if (RightPress_Helper::is_empty($value['value']) && ($this->context_is('order_field') || $this->context_is('product_prop'))) {
            ...
            $this->delete_stored_value($item_id);
        ...
        $this->store_in_meta($item_id, $this->get_value_access_key($quantity_index), $value['value']);

        // Store field id as hidden meta
        $this->store_in_meta($item_id, $this->get_id_access_key($quantity_index), $this->get_id());
        ...
        $this->store_in_meta($item_id, $this->get_extra_data_access_key($quantity_index), $value['data']);

        // Store file data
        foreach ($value['files'] as $access_key => $file_data) {

            // Store file in meta
            $this->store_in_meta($item_id, WCCF_Field::get_file_data_access_key($access_key), $file_data);

            // Remove temporary file from meta, if any
            $this->delete_from_meta($item_id, WCCF_Field::get_temp_file_data_access_key($access_key));
     ...
     * @return void
     */
    protected function delete_stored_value($item_id)
    {
        // Delete value entry
        $this->delete_from_meta($item_id, $this->get_value_access_key());

        // Delete id entry
        $this->delete_from_meta($item_id, $this->get_id_access_key());

        // Delete extra data entry
        $this->delete_from_meta($item_id, $this->get_extra_data_access_key());
    }
     ...
     * @return bool
     */
    public function store_in_meta($item_id, $key, $value)
    {
        // Store product field data in order item meta
        if ($this->context_is('product_field')) {
            $result = wc_update_order_item_meta($item_id, $key, $value);
        }
        // Store user field data in user meta
        else if ($this->context_is('user_field')) {
            $result = update_user_meta($item_id, $key, $value);
        }
        // Store all other field data in post meta
        else {
            $result = update_post_meta($item_id, $key, $value);
        }
     ...
     * @return bool
     */
    protected function delete_from_meta($item_id, $key)
    {
        // Delete from order item meta
        if ($this->context_is('product_field')) {
            $result = wc_delete_order_item_meta($item_id, $key);
        }
        // Delete from user meta
        else if ($this->context_is('user_field')) {
            $result = delete_user_meta($item_id, $key);
        }
        // Delete from regular post meta
        else {
            $result = delete_post_meta($item_id, $key);
        }
