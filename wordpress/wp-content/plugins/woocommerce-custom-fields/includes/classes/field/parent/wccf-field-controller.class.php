...
class WCCF_Field_Controller extends WCCF_Post_Object_Controller
{
    ...
    public function process_duplicate()
    {
        ...
        try {
            ...
            if (!isset($_REQUEST['wccf_field_key']) || RightPress_Helper::is_empty($_REQUEST['wccf_field_key']) || !is_string($_REQUEST['wccf_field_key'])) {
                throw new Exception(__('New field key is not valid.', 'rp_wccf'));
            }
            ...
            if ($this->key_exists($key)) {
                throw new Exception(__('New field key is not unique.', 'rp_wccf'));
            }
    ...
    public static function print_field($field, $item_id, $quantity_index)
    {
        ...
        $attributes = array(
            ...
            'name'              => 'wccf[' . $context . '][' . $field_id_for_name . ']' . ($field->accepts_multiple_values() ? '[]' : ''),
        ...
        foreach ($fields as $field) {
            ...
            for ($i = $i; $i < $iterations; $i++) {
                ...
                if (empty($value) && $value !== '0') {
                    ...
                    // Validate value
                    $validation_result = self::validate_field_value($field, $value, $item_id);
                ...
                if ($field->field_type_is('file') && !empty($value) && !$empty_error && !$value_error && !defined('WCCF_UPLOADING_FILE')) {
                    ...
                    foreach ($value as $index => $access_key) {
                        ...
                        // Attempt to load file data from post meta
                            ...
                            // Get meta context
                            $meta_context = WCCF_Field_Controller::get_meta_context($field->get_context());

                            // Check for just uploaded files
                            $file_data = maybe_unserialize(RightPress_Helper::get_meta($meta_access_key, $meta_context, $item_id));

                            // Check for existing files, used for order fields and product properties
                            if ($file_data === false && ($field->context_is('order_field') || $field->context_is('product_prop') || $field->context_is('user_field'))) {
                                $file_data = maybe_unserialize(RightPress_Helper::get_meta(WCCF_Field::get_file_data_access_key($access_key), $meta_context, $item_id));
                            }
                        }
     ...
     * @return mixed
     */
    public static function validate_field_value($field, $value, $item_id = null)
    {
        ...
        if ($field->field_type_is('file') && empty($value)) {
            if (!$field->is_required() || ($item_id && $field->get_stored_value($item_id))) {
                return true;
            }
            else {
        ...
        try {
            ...
            switch ($field_type) {
                ...
                case 'checkbox':
                case 'radio':
                    return WCCF_FB::$method($value, $field->get_options_list());
                    break;

                default:
                    return WCCF_FB::$method($value);
                    break;
            }
        }
        catch (Exception $e) {
     ...
     * @return void
     */
    public static function store_field_values($post_id, $context, $skip_invalid = false, $is_checkout = false)
    {
        // Special handling of user fields on checkout
        if ($context === 'user_field' && $is_checkout) {

            // Take reference of order id and load order object
            $order_id = $post_id;
            $order = RightPress_Helper::wc_get_order($order_id);

            // Check if user id is set
            if ($order && $order->get_user_id()) {
                $post_id = $order->get_user_id();
                $store_value = true;
            }
            else {
                $store_value = false;
        ...
        foreach ($values as $field_id => $field_value) {
            ...
            // Store value
            if ($store_value) {
                $field->store_value($post_id, $field_value);
            }
            ...
            if ($context === 'user_field' && $is_checkout) {
                ...
                update_post_meta($order_id, $field->get_value_access_key(), $field_value['value']);

                // Store field id as hidden meta
                update_post_meta($order_id, $field->get_id_access_key(), $field->get_id());
                ...
                update_post_meta($order_id, $field->get_extra_data_access_key(), $field_value['data']);

                // Store file data
                foreach ($field_value['files'] as $access_key => $file_data) {

                    // Store file in meta
                    update_post_meta($order_id, WCCF_Field::get_file_data_access_key($access_key), $file_data);
                }
            }
        }
        ...
        if (RightPress_Helper::post_type_is($post_id, 'product')) {
            WCCF_WC_Product::clear_cached_price($post_id);
        }
    }
    ...
    public static function get_field_values_for_frontend($fields, $item_id, $context)
    {
        ...
        foreach ($fields as $field) {
            ...
            // Get field value
            if ($supports_visibility && WCCF_Settings::get('display_default_' . $context . '_values')) {
                $field_value = $field->get_final_value($item_id);
            }
            else {
                $field_value = $field->get_stored_value($item_id);
            }
    ...
    /**
     * Get meta context from field context
     *
     * @access public
     * @param string $field_context
     * @return mixed
     */
    public static function get_meta_context($field_context)
    {
