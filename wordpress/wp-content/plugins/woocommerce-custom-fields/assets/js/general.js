...
jQuery(document).ready(function() {
    ...
    if (!wccf_general_config.is_backend) {
        ...
        /**
         * Dynamic price update on product field configuration change
         */
        if (wccf_general_config.display_total_price && jQuery('form.cart .wccf_product_field[data-wccf-pricing]')) {

            var form = jQuery('form.cart:has(.wccf_product_field)');

            // Iterate over fields that have pricing and set up pricing updates
            form.find('.wccf_product_field[data-wccf-pricing]').each(function() {
                set_up_pricing_updates_for_field(jQuery(this), form);
            });

            // Handle default fields
            wccf_update_price(form);
        }
         ...
         * Only needs to be performed if multiple file upload field is present
         */
        jQuery('.wccf_field_container input[multiple].wccf_file').first().each(function() {
        ...
        if (jQuery('form.variations_form div[data-wccf-uses-attribute-conditions]').length) {
            ...
            jQuery('.variations_form').on('woocommerce_variation_has_changed', function(event) {
                ...
                // Check only update price if all variation attributes are selected
                if (variation_attributes_selected()) {

                    // Update price
                    wccf_update_price(jQuery('.variations_form'));
                }
                else {
     ...
     * Update price
     */
    function wccf_update_price(form)
    {
        // Serialize form data
        var form_data = form.serialize();

        // File upload field values are not serialized so we need to take them manually
        var file_upload_values = {};

        form.find('input.wccf_file').each(function() {
            file_upload_values[jQuery(this).prop('name')] = jQuery(this).val();
        });

        var file_upload_data = jQuery.param(file_upload_values);

        // Append file upload values to serialized form data
        form_data += (form_data !== '' && file_upload_data !== '') ? '&' : '';
        ...
        jQuery.post(
            ...
            {
                action:     'wccf_product_field_price_update',
                data:       form_data
            },
            function(response) {
                ...
                // Check for errors
                if (typeof response === 'object' && typeof response.result !== 'undefined' || response.result === 'success') {

                    // Replace price
                    jQuery('form.cart dl.wccf_grand_total dd').html(response.price_html);

                    // Show grand total
                    if (form.hasClass('variations_form') && variation_attributes_selected()) {
                        show_grand_total();
                    }
                }
            }
    ...
    /**
     * Set up pricing update for single field
     */
    function set_up_pricing_updates_for_field(field, form)
    {
    ...
    /**
     * Check if all variations have attributes selected
     */
    function variation_attributes_selected()
    {
        var selected = true;

        // Iterate over attributes
        jQuery('.variations_form select[name^="attribute_pa_"]').each(function() {

            // Attribute does not have value selected
            if (jQuery(this).val() === '') {
                selected = false;
                return;
            }
    ...
    {
        jQuery('.variations_form .wccf_grand_total').show();
    }

    /**
     * Hide grand total
     */
    function hide_grand_total()
    {
        jQuery('.variations_form .wccf_grand_total').hide();
    }
                ...
                if (response.fields.length) {
                    ...
                    jQuery.each(response.fields, function(index, field) {
                        ...
                        // Field does not exist
                            ...
                            new_field.find('.wccf_product_field[data-wccf-pricing]').each(function() {
                                set_up_pricing_updates_for_field(jQuery(this), form);
                            });
                            ...
                            // Set customer id cookie if not exists (required for multiple file uploads via ajax)
                            new_field.find('input[multiple].wccf_file').first().each(function() {
                                maybe_set_customer_id_cookie();
                            });
    ...
    {
        var fields_with_pricing_affected = false;
            ...
            if (typeof window[conditions] !== 'undefined') {
                ...
                if (conditions_match && !form.find('#' + id).is(':visible')) {
                    jQuery('#' + id).closest('.wccf_field_container, .wccf_meta_box_field_container, .wccf_user_profile_field_container').each(function() {
                        ...
                        // Check if field has pricing
                        if (jQuery(this).find('.wccf[data-wccf-pricing]').length) {
                            fields_with_pricing_affected = true;
                        }
                    });
                }
                // Make field hidden
        ...
        if (fields_with_pricing_affected) {
            wccf_update_price(form);
        }
    }
        ...
        field.fileupload({
            ...
            progress: function (e, data) {
                ...
                field_container.find('.wccf_file_upload_status_' + data.wccf_upload_progress_id).animate({'opacity': 0}, 500, function() {
                    ...
                    // Update product field price
                    if (field_container.find('.wccf[data-wccf-pricing]').length) {
                        wccf_update_price(field_container.closest('form.cart'));
                    }
